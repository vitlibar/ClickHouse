...
| `timestamp_codec` | Codec | none | Compression codec used for the `timestamp` column in the inner [samples](#samples-table) table. Can also be set by specifying the `CODEC` of the `timestamp` column explicitly |
| `scalar_codec` | Codec | none | Compression codec used for the `value` column in the inner [samples](#samples-table) table. Can also be set by specifying the `CODEC` of the `value` column explicitly |
| `id_codec` | Codec | none | Compression codec used for the `id` column in the inner [samples](#samples-table) and [tags](#tags-table) tables. Can also be set by specifying the `CODEC` of the `id` column explicitly |
