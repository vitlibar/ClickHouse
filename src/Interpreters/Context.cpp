...
void Context::setCurrentProfileImpl(const String & profile_name, bool internal)
{
    ...
    removeDuplicatesFromProfileIDs(enabled_profiles);
}
...
void Context::setDefaultProfiles(const Poco::Util::AbstractConfiguration & config)
{
    ...
    setCurrentProfileImpl(shared->system_profile_name, true);

    applySettingsQuirks(settings, &Poco::Logger::get("SettingsQuirks"));

    shared->buffer_profile_name = config.getString("buffer_profile", shared->system_profile_name);
    buffer_context = Context::createCopy(shared_from_this());
    buffer_context->setCurrentProfileImpl(shared->buffer_profile_name, true);
}
