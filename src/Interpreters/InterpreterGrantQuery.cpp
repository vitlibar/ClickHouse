...
namespace
{
        ...
        const std::shared_ptr<const ContextAccess> & context)
    {
        if (context && !context->hasGrantOption(access_to_revoke))
        {
            ...
            context->checkGrantOption(filtered_access_to_revoke);
        }
    ...
    template <typename T>
    void updateFromQueryImpl(T & grantee, const ASTGrantQuery & query, const RolesOrUsersSet & roles_from_query,
                             const std::shared_ptr<const ContextAccess> & context)
    {
        ...
        if (!roles_from_query.empty())
        {
            ...
            else
            {
                ...
                doRevokeRoles(grantee.granted_roles, grantee_default_roles, roles_from_query, query.admin_option, context);
            }
        }
    }
}
...
BlockIO InterpreterGrantQuery::execute()
{
    ...
    auto update_func = [&](const AccessEntityPtr & entity) -> AccessEntityPtr
    {
        ...
        {
            updateFromQueryImpl(*user, query, roles_from_query, access);
            return user;
        }
        else if (auto role = typeid_cast<std::shared_ptr<Role>>(clone))
        {
            updateFromQueryImpl(*role, query, roles_from_query, access);
            return role;
        }
        else
            return entity;
