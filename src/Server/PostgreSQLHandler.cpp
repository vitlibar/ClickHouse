...
void PostgreSQLHandler::processQuery()
{
    try
    {
        ...
        const auto & settings = session->sessionContext()->getSettings();
        ...
        {
            secret_key = dis(gen);
            session->getClientInfo().current_query_id = Poco::format("postgres:%d:%d", connection_id, secret_key);
            auto query_context = session->makeQueryContext();
            query_context->setDefaultFormat("PostgreSQLWire");
