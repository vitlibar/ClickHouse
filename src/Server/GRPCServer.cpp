...
namespace
{
    ...
    void CallDataQuery::parseQuery()
    {
        ...
        if (!request.format().empty())
        {
            ...
            query_context->setDefaultFormat(request.format());
        }
    ...
    void CallDataQuery::parseData()
    {
        ...
        if (io.out)
        {
            if (!insert_query || !(insert_query->data || !request.input_data().empty() || request.use_next_input_data()))
            {
            ...
            while (auto block = res_stream->read())
                io.out->write(block);
            if (request.use_next_input_data())
            {
                status = READ_DATA;
                responder.Read(&request, static_cast<void *>(this));
                return;
