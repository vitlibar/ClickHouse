...
#include <Common/SettingChange.h>
#include <Common/SettingField.h>
...
public:
    ...
    {
    public:
        Iterator(typename Traits::Data & data_, size_t index_, bool skip_unchanged_);
        Iterator & operator++();
        Iterator operator++(int);

        std::string_view getName() const { return Traits::getName(index); }
        ...
        std::pair<std::string_view, Field> operator*() const { return {getName(), getValue()}; }
...
template <typename Traits_>
BaseSettings<Traits_>::Iterator::Iterator(typename Traits::Data & data_, size_t index_, bool skip_unchanged_)
...
void BaseSettings<Traits_>::write(WriteBuffer & out, SettingsWriteFormat format) const
{
    ...
    {
        BaseSettingsHelper::writeString(it.getName(), out);
        if (format >= SettingsWriteFormat::STRINGS_WITH_FLAGS)
        {
            BaseSettingsHelper::writeFlag(Traits::isImportant(it.getIndex()), out);
            BaseSettingsHelper::writeString(Traits::getValueAsString(*this, it.getIndex()), out);
        }
        else
            Traits::writeBinary(*this, it.getIndex(), out);
    }
...
#define DECLARE_SETTINGS_TRAITS(SETTINGS_TRAITS_NAME, LIST_OF_SETTINGS_MACRO) \
    ...
    { \
        ...
        static std::string_view getName(size_t index); \
...
#define IMPLEMENT_SETTINGS_TRAITS(SETTINGS_TRAITS_NAME, LIST_OF_SETTINGS_MACRO) \
    ...
    static std::string_view SETTINGS_TRAITS_NAME::getName(size_t index) { return SETTINGS_TRAITS_NAME##Info::instance().field_infos[index].name; } \
