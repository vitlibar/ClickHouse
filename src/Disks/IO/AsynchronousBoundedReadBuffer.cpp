...
{
    size_t read_end_position = read_until_position ? *read_until_position : impl->getFileSize();
    if (file_offset_of_buffer_end > read_end_position)
        throwReadBeyondLastOffset();
    return chooseBufferSizeForRemoteReading(read_settings, read_end_position - file_offset_of_buffer_end);
}
