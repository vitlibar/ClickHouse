...
public:
    ...
    UInt128 toUInt128() const { return counter; }
    ...
    String toBigEndian() const;

    /// Converts a string of 16 bytes length in big endian order to a counter.
    static InitVector fromBigEndian(const String & str_);
...
public:
    ...
    /// We chose to use CTR cipther algoritms because they have the following important features:
    ...
    /// Checks whether a passed key length is supported, i.e. it has length 128 or 192 or 256.
    static void checkKeyLengthIsSupported(size_t key_length);
