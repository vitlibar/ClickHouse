    ...
    std::optional<ProfileEvents::Event> profile_event)
{
    chassert(
        !blob_storage_log
        || ((keys.size() == local_paths_for_blob_storage_log.size()) && (keys.size() == file_sizes_for_blob_storage_log.size())));
    ...
    if (try_batch_delete)
    {
        ...
        while (current_position < keys.size())
        {
            ...
            else
            {
                ...
                /// or b) all the objects didn't exist; or c) some failure occured.
                const auto & err = outcome.GetError();
