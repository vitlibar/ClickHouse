...
size_t encryptBlocks(EVP_CIPHER_CTX * evp_ctx, const char * data, size_t size, WriteBuffer & out)
{
    ...
    while (in_size < size)
    {
        size_t part_size = std::min(size - in_size, kBlockSize);
...
size_t encryptBlockWithPadding(EVP_CIPHER_CTX * evp_ctx, const char * data, size_t size, size_t pad_left, WriteBuffer & out)
{
    ...
    memset(&padded_data[0], 0, pad_left);
...
String InitVector::toBigEndian() const
{
...
InitVector InitVector::fromBigEndian(const String & str)
{
...
{
    String str(kSize, 0);
    in.readStrict(str.data(), kSize);
    *this = fromBigEndian(str);
}

void InitVector::write(WriteBuffer & out) const
{
    String str = toBigEndian();
...
InitVector InitVector::random()
{
    ...
    UInt128 counter;
    UInt8 * ptr = reinterpret_cast<UInt8 *>(&counter);
    UInt8 * end = ptr + sizeof(counter);
    while (ptr < end)
    {
        auto value = dis(gen);
        size_t part_size = std::min(static_cast<size_t>(end - ptr), sizeof(value));
        memcpy(ptr, &value, part_size);
        ptr += part_size;
    }
    ...
    , init_vector(iv_)
{
    ...
    if (key_.length() != static_cast<size_t>(EVP_CIPHER_key_length(evp_cipher)))
...
void Encryptor::checkKeyLengthIsSupported(size_t key_length)
{
    if ((key_length == 16) || (key_length == 24) || (key_length == 32))
        return;
    throw Exception(ErrorCodes::DATA_ENCRYPTION_ERROR, "Key length {} is not supported, supported only keys of length 128, 192, or 256 bits", key_length);
}
...
void Encryptor::encrypt(const char * data, size_t size, WriteBuffer & out)
{
    ...
    auto current_iv = (init_vector + blocks(offset)).toBigEndian();
