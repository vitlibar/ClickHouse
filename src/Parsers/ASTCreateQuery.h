...
public:
    ...
    ASTTargetTables * target_tables = nullptr;
    ...
    const ASTTargetTables::Target & getTarget(TargetTableKind target_table_kind = TargetTableKind::kTarget) const;
    const StorageID & getTargetTableId(TargetTableKind target_table_kind = TargetTableKind::kTarget) const;

    void setUUIDs(const CreateQueryUUIDs & uuids);
    CreateQueryUUIDs generateRandomUUIDs(bool always_generate_new_uuids = false);
...
protected:
    ...
    void forEachPointerToChild(std::function<void(void**)> f) override
    {
        ...
        f(reinterpret_cast<void **>(&target_tables));
