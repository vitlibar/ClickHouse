...
namespace
{
    ...
    void formatName(const DatabaseAndTableName & name, bool name_is_in_temp_db, ElementType type, bool show_type, const IAST::FormatSettings & format)
    {
        switch (type)
        {
    ...
    {
        formatName(element.name, element.name_is_in_temp_db, element.type, true, format);
        ...
        if (new_name_is_different)
        {
            ...
            formatName(element.new_name, element.new_name_is_in_temp_db, element.type, show_type, format);
        }
    }
