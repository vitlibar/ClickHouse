...
public:
    ...
    const ViewTarget & getTarget(TargetKind kind = TargetKind::Default) const;
    ...
    const StorageID & getTableID(TargetKind kind = TargetKind::Default) const { return getTarget(kind).table_id; }
    bool hasTableID(TargetKind kind = TargetKind::Default) const { return !getTableID(kind).empty(); }
    ...
    const UUID & getInnerUUID(TargetKind kind = TargetKind::Default) const { return getTarget(kind).inner_uuid; }
    ...
    std::shared_ptr<ASTStorage> getTableEngine(TargetKind kind = TargetKind::Default) const { return getTarget(kind).table_engine; }
