...
public:
    ...
    /// Locates a time range in a range selector, for example for "[1h30m]" the function locates "1h30m".
    static bool trySplitSelectorRange(std::string_view input,
                                      size_t & res_range_start_pos,
                                      size_t & res_range_length,
                                      String * error_message = nullptr,
                                      size_t * error_pos = nullptr);

    /// Locates a range and optionally a resolution in a subquery, for example for "[1h:5m]" the function locates "1h" and "5m".
    static bool trySplitSubqueryRange(std::string_view input,
                                      size_t & res_range_start_pos,
                                      size_t & res_range_length,
