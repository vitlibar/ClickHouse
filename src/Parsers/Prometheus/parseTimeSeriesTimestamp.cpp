...
namespace
{
    ...
    {
        Float64 scaled_value = float_value * DecimalUtils::scaleMultiplier<T>(scale);
        if ((scaled_value > std::numeric_limits<typename T::NativeType>::max()) ||
            (scaled_value < std::numeric_limits<typename T::NativeType>::min()))
        {
            throw Exception(ErrorCodes::DECIMAL_OVERFLOW,
                            "Cannot convert {} to {}: Overflow, the number is too big",
