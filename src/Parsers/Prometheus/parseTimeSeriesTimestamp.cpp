...
namespace
{
    ...
    {
        T result;
        if (common::mulOverflow(int_value, DecimalUtils::scaleMultiplier<Int64>(scale), result.value))
        {
    ...
    {
        return static_cast<Int64>(float_value * DecimalUtils::scaleMultiplier<Int64>(scale));
    }
    ...
    T getFromIntervalKind(Int64 intervals, IntervalKind interval_kind, UInt32 scale)
    {
        ...
        switch (interval_kind.kind)
        {
            ...
            {
                if (scale > 9)
                    scale_multiplier = DecimalUtils::scaleMultiplier<Int64>(scale - 9);
                else if (scale < 9)
                    scale_divisor = DecimalUtils::scaleMultiplier<Int64>(9 - scale);
                break;
            }
            case IntervalKind::Kind::Microsecond:
            {
                if (scale > 6)
                    scale_multiplier = DecimalUtils::scaleMultiplier<Int64>(scale - 6);
                else if (scale < 6)
                    scale_divisor = DecimalUtils::scaleMultiplier<Int64>(6 - scale);
                break;
            }
            case IntervalKind::Kind::Millisecond:
            {
                if (scale > 3)
                    scale_multiplier = DecimalUtils::scaleMultiplier<Int64>(scale - 3);
                else if (scale < 3)
                    scale_divisor = DecimalUtils::scaleMultiplier<Int64>(3 - scale);
                break;
            }
            case IntervalKind::Kind::Second:
            {
                scale_multiplier = DecimalUtils::scaleMultiplier<Int64>(scale);
                break;
            }
