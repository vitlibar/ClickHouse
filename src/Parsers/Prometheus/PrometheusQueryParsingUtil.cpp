...
namespace
{
    ...
    bool tryParseNumberFormat(std::string_view input, UInt32 scale, T & result, String * error_message, size_t * error_pos)
    {
        ...
        if constexpr (is_decimal<T>)
        {
            ...
            /// TODO: Excessive digits shouldn't be just cut off, it's more correct to check the first excessive digit
            /// and round up the result if it's 5..9
            UInt32 target_scale = scale;
            if (!tryReadDecimalText(buf, result, std::numeric_limits<typename T::NativeType>::digits10, target_scale))
            {
    ...
    bool tryParseHexFormat(std::string_view input, UInt32 scale, T & result, String * error_message, size_t * error_pos)
    {
        ...
        if constexpr (is_decimal<T>)
        {
            if (common::mulOverflow(value, DecimalUtils::scaleMultiplier<Int64>(scale), result.value))
            {
    ...
    bool tryParseDurationFormat(std::string_view input, UInt32 scale, T & result, String * error_message, size_t * error_pos)
    {
        ...
        while (pos != input.length())
        {
            ...
            {
                Int64 add_seconds;
                if (common::mulOverflow(number, seconds_per_unit, add_seconds) || common::addOverflow(add_seconds, seconds, seconds))
                {
                    setErrorMessage(error_message,
            ...
            {
                Int64 add_ms;
                if (common::mulOverflow(number, ms_per_unit, add_ms) || common::addOverflow(add_ms, milliseconds, milliseconds))
                {
                    setErrorMessage(error_message,
                                    "Cannot parse {} {} in duration format: Overflow, the duration is too big",
                                    getTypeName<T>(), quoteString(input));
        ...
        if constexpr (is_decimal<T>)
        {
            ...
            if (scale > 3)
            {
                if (common::mulOverflow(milliseconds, DecimalUtils::scaleMultiplier<Int64>(scale - 3), scaled_milliseconds))
                {
                    setErrorMessage(error_message,
            ...
            {
                Int64 divisor = DecimalUtils::scaleMultiplier<Int64>(3 - scale);
            ...
            Int64 scaled_seconds;
            if (common::mulOverflow(seconds, DecimalUtils::scaleMultiplier<Int64>(scale), scaled_seconds)
                || common::addOverflow(scaled_seconds, milliseconds, result.value))
            {
                setErrorMessage(error_message,
                                "Cannot parse {} {}: Overflow, the duration is too big",
                                getTypeName<T>(), quoteString(input));
...
bool PrometheusQueryParsingUtil::tryParseDuration(
    std::string_view input, UInt32 duration_scale, DurationType & res_duration, String * error_message, size_t * error_pos)
{
    return tryParseNumber(input, duration_scale, res_duration, error_message, error_pos);
}


/// Parses a time range which is used in range selectors.
bool PrometheusQueryParsingUtil::trySplitSelectorRange(
    std::string_view input, size_t & res_range_start_pos, size_t & res_range_length, String * error_message, size_t * error_pos)
{
    ...
    res_range_start_pos = start_pos;
    res_range_length = end_pos - start_pos;
    return true;
}

/// Parses a time range with an optional resolution which are used in subqueries.
bool PrometheusQueryParsingUtil::trySplitSubqueryRange(
    std::string_view input,
    size_t & res_range_start_pos,
    size_t & res_range_length,
    size_t & res_resolution_start_pos,
    size_t & res_resolution_length,
    ...
    size_t * error_pos)
{
    ...
    res_range_start_pos = range_start_pos;
    res_range_length = range_end_pos - range_start_pos;
    res_resolution_start_pos = resolution_start_pos;
    res_resolution_length = resolution_end_pos - resolution_start_pos;
    return true;
}
