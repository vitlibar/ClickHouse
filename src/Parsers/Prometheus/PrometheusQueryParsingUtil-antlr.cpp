    ...
    private:
        ...
        Node * makeOffset(antlr4_grammars::PromQLParser::OffsetOpContext * ctx, Node * expression)
        {
            ...
            if (auto * timestamp_ctx = ctx->TIMESTAMP())
                ok &= parseTimestamp(timestamp_ctx, new_node->at_timestamp.emplace());

            if (auto * offset_value_ctx = ctx->OFFSET_VALUE())
                ok &= parseDuration(offset_value_ctx, new_node->offset_value.emplace());
        ...
        std::any aggregateResult(std::any aggregate, std::any next_result) override
        {
            ...
            if (node && next_node)
                throw Exception(ErrorCodes::LOGICAL_ERROR, "Can't aggregate nodes \n{}and\n{}", node->node_type, next_node->node_type);
            if (node)
                return node;
            else
                return next_node;
...
bool PrometheusQueryParsingUtil::tryParseQuery([[maybe_unused]] std::string_view input, [[maybe_unused]] UInt32 timestamp_scale, [[maybe_unused]] PrometheusQueryTree & result, [[maybe_unused]] String * error_message, [[maybe_unused]] size_t * error_pos)
{
#if USE_ANTLR4_GRAMMARS
