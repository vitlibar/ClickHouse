...
namespace
{
    ...
    private:
        ...
        bool parseDuration(size_t start_pos, size_t length, DurationType & result)
        {
            String error_message;
            size_t error_pos;
            if (!PrometheusQueryParsingUtil::tryParseDuration(getText(start_pos, length), timestamp_scale, result, &error_message, &error_pos))
            {
        ...
        bool splitSelectorRange(const antlr4::tree::TerminalNode * ctx, size_t & res_range_start_pos, size_t & res_range_length)
        {
            String error_message;
            size_t error_pos;
            if (!PrometheusQueryParsingUtil::trySplitSelectorRange(getText(ctx), res_range_start_pos, res_range_length, &error_message, &error_pos))
            {
                error_listener.setError(error_message, error_pos + getStartPos(ctx));
                return false;
            }
            res_range_start_pos += getStartPos(ctx);
            return true;
        }

        bool splitSubqueryRange(const antlr4::tree::TerminalNode * ctx, size_t & res_range_start_pos, size_t & res_range_length, size_t & res_resolution_start_pos, size_t & res_resolution_length)
        {
            String error_message;
            size_t error_pos;
            if (!PrometheusQueryParsingUtil::trySplitSubqueryRange(getText(ctx), res_range_start_pos, res_range_length, res_resolution_start_pos, res_resolution_length, &error_message, &error_pos))
            {
                error_listener.setError(error_message, error_pos + getStartPos(ctx));
                return false;
            }
            res_range_start_pos += getStartPos(ctx);
            res_resolution_start_pos += getStartPos(ctx);
            return true;
        }
        ...
        using Timestamp = PrometheusQueryTree::Timestamp;
        ...
        Node * makeScalarNode(antlr4::tree::TerminalNode * ctx)
        {
            ScalarType scalar;
        ...
        Node * makeTimestampNode(antlr4::tree::TerminalNode * ctx)
        {
            TimestampType timestamp;
            if (!parseTimestamp(ctx, timestamp))
            {
                chassert(error_listener.hasError());
                return nullptr;
            }
            auto new_node = std::make_unique<Timestamp>();
            new_node->start_pos = getStartPos(ctx);
            new_node->length = getLength(ctx);
        ...
        Node * makeDurationNode(antlr4::tree::TerminalNode * ctx, int sign = 1)
        {
            return makeDurationNode(getStartPos(ctx), getLength(ctx), sign);
        }

        Node * makeDurationNode(size_t start_pos, size_t length, int sign = 1)
        {
            DurationType duration;
            if (!parseDuration(start_pos, length, duration))
            {
                chassert(error_listener.hasError());
                return nullptr;
            }
        ...
        Node * makeRangeSelector(antlr4_grammars::PromQLParser::MatrixSelectorContext * ctx)
        {
            ...
            if (!splitSelectorRange(selector_range_ctx, range_start_pos, range_length))
            {
                chassert(error_listener.hasError());
                return nullptr;
            }

            auto range_node = makeDurationNode(range_start_pos, range_length);
            if (!range_node)
            {
                chassert(error_listener.hasError());
                return nullptr;
        ...
        Node * makeSubquery(antlr4_grammars::PromQLParser::SubqueryOpContext * ctx, Node * expression)
        {
            ...
            if (!splitSubqueryRange(subquery_range_ctx, range_start_pos, range_length, resolution_start_pos, resolution_length))
            {
                chassert(error_listener.hasError());
                return nullptr;
            }
            ...
            if (resolution_length)
            {
                resolution_node = makeDurationNode(resolution_start_pos, resolution_length);
                if (!resolution_node)
                {
                    chassert(error_listener.hasError());
                    return nullptr;
        ...
        Node * makeAt(antlr4_grammars::PromQLParser::OffsetOpContext * ctx, Node * expression)
        {
            ...
            if (ctx->AT())
            {
                at_node = makeTimestampNode(ctx->TIMESTAMP());
                if (!at_node)
                {
                    chassert(error_listener.hasError());
                    return nullptr;
            ...
            if (ctx->OFFSET())
            {
                int sign = ctx->SUB() ? -1 : 1;
                offset_node = makeDurationNode(ctx->DURATION(), sign);
                if (!offset_node)
                {
                    chassert(error_listener.hasError());
                    return nullptr;
        ...
        std::any visitLiteral(antlr4_grammars::PromQLParser::LiteralContext * ctx) override
        {
            if (auto * number_ctx = ctx->NUMBER())
                return makeScalarNode(number_ctx);
...
bool PrometheusQueryParsingUtil::tryParseQuery([[maybe_unused]] std::string_view input,
                                               [[maybe_unused]] UInt32 timestamp_scale,
                                               [[maybe_unused]] PrometheusQueryTree & result,
