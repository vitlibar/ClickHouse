...
public:
    ...
    /// The timestamp type is used to represent timestamps, for example "@1736046605".
    using TimestampType = DateTime64;

    /// The duration type is used to represent durations or offsets, for example "[1d30m]" or "offset -2h".
    ...
    /// A scalar literal, i.e. a floating-point or an integer number, or Inf/NaN.
    /// Examples: -2.43, 123, +Inf, 2h30m.
    ...
    /// Timestamp is specified after `@`.
    class Timestamp : public Node
    {
    public:
        TimestampType timestamp;
        Timestamp() { node_type = NodeType::Timestamp; result_type = ResultType::SCALAR; }
        Node * clone(std::vector<std::unique_ptr<Node>> & node_list_) const override;
        String dumpNode(const PrometheusQueryTree & tree, size_t indent) const override;
    };

    /// Duration can be specified in range selector, subquery, or after the `offset` keyword.
    ...
    /// Returns the scale used to represent timestamps and durations in this tree.
