...
namespace
{
    using Node = PrometheusQueryTreeBase::Node;
...
Node * PrometheusQueryTreeBase::ScalarLiteral<ScalarType>::clone(std::vector<std::unique_ptr<Node>> & node_list_) const
{
    return cloneNodeImpl(this, node_list_);
}

Node * PrometheusQueryTreeBase::StringLiteral::clone(std::vector<std::unique_ptr<Node>> & node_list_) const
{
    return cloneNodeImpl(this, node_list_);
}

Node * PrometheusQueryTreeBase::Matcher::clone(std::vector<std::unique_ptr<Node>> & node_list_) const
{
    return cloneNodeImpl(this, node_list_);
}

template <typename TimeType>
Node * PrometheusQueryTreeBase::InstantSelector<TimeType>::clone(std::vector<std::unique_ptr<Node>> & node_list_) const
{
    return cloneNodeImpl(this, node_list_);
}

template <typename TimeType>
Node * PrometheusQueryTreeBase::RangeSelector<TimeType>::clone(std::vector<std::unique_ptr<Node>> & node_list_) const
{
    return cloneNodeImpl(this, node_list_);
}

template <typename TimeType>
Node * PrometheusQueryTreeBase::Subquery<TimeType>::clone(std::vector<std::unique_ptr<Node>> & node_list_) const
{
    return cloneNodeImpl(this, node_list_);
}

Node * PrometheusQueryTreeBase::Function::clone(std::vector<std::unique_ptr<Node>> & node_list_) const
{
    return cloneNodeImpl(this, node_list_);
}

Node * PrometheusQueryTreeBase::UnaryOperator::clone(std::vector<std::unique_ptr<Node>> & node_list_) const
{
    return cloneNodeImpl(this, node_list_);
}

Node * PrometheusQueryTreeBase::BinaryOperator::clone(std::vector<std::unique_ptr<Node>> & node_list_) const
{
    return cloneNodeImpl(this, node_list_);
}

Node * PrometheusQueryTreeBase::AggregationOperator::clone(std::vector<std::unique_ptr<Node>> & node_list_) const
{
    return cloneNodeImpl(this, node_list_);
}

template <typename TimeType, typename ScalarType>
Node * PrometheusQueryTreeBase::ConstInstantVector<TimeType, ScalarType>::clone(std::vector<std::unique_ptr<Node>> & node_list_) const
{
    return cloneNodeImpl(this, node_list_);
}

template <typename TimeType, typename ScalarType>
Node * PrometheusQueryTreeBase::ConstRangeVector<TimeType, ScalarType>::clone(std::vector<std::unique_ptr<Node>> & node_list_) const
{
    return cloneNodeImpl(this, node_list_);
}

PrometheusQueryTreeBase & PrometheusQueryTreeBase::operator=(const PrometheusQueryTreeBase & src)
{
    if (this != &src)
    {
        ...
        *this = PrometheusQueryTreeBase{src.promql_query, new_root, std::move(new_node_list)};
    }
    return *this;
}

PrometheusQueryTreeBase & PrometheusQueryTreeBase::operator=(PrometheusQueryTreeBase && src)
{
...
PrometheusQueryResultType PrometheusQueryTreeBase::getResultType() const
{
    if (!root)
...
namespace
{
    ...
    template <typename ScalarType>
    String scalarToString(ScalarType scalar)
    {
        return toString(scalar);
    }

    template <typename TimeType>
    String timestampToString(TimeType timestamp)
    {
        return toString(timestamp);
    }

    template <typename TimeType>
    String durationToString(TimeType duration)
    {
        ...
        return toString(static_cast<Float64>(duration));
    }
}

String PrometheusQueryTreeBase::dumpTree(size_t indent) const
{
    if (root)
...
template <typename ScalarType>
String PrometheusQueryTreeBase::ScalarLiteral<ScalarType>::dumpTree(size_t indent) const
{
    return fmt::format("{}ScalarLiteral({})\n", makeIndent(indent), scalarToString(scalar));
}

String PrometheusQueryTreeBase::StringLiteral::dumpTree(size_t indent) const
{
    return fmt::format("{}StringLiteral({})\n", makeIndent(indent), quoteString(string));
}

String PrometheusQueryTreeBase::Matcher::dumpTree(size_t indent) const
{
    return fmt::format("{}Matcher({} {} \"{}\")\n", makeIndent(indent), label_name, matcher_type, label_value);
}

template <typename TimeType>
String PrometheusQueryTreeBase::InstantSelector<TimeType>::dumpTree(size_t indent) const
{
    const auto & matchers = this->getMatchers();
    String str = fmt::format("{}InstantSelector(", makeIndent(indent));
    if (this->at)
        str += fmt::format("at {}", timestampToString(*this->at));
    if (this->at && static_cast<bool>(this->offset))
        str += ", ";
    if (static_cast<bool>(this->offset))
        str += fmt::format("offset {}", durationToString(this->offset));
...
template <typename TimeType>
String PrometheusQueryTreeBase::RangeSelector<TimeType>::dumpTree(size_t indent) const
{
    const auto & matchers = this->getMatchers();
...
template <typename TimeType>
String PrometheusQueryTreeBase::Subquery<TimeType>::dumpTree(size_t indent) const
{
    ...
    if (at)
        str += fmt::format(", at {}", timestampToString(*at));
...
String PrometheusQueryTreeBase::Function::dumpTree(size_t indent) const
{
...
String PrometheusQueryTreeBase::UnaryOperator::dumpTree(size_t indent) const
{
...
String PrometheusQueryTreeBase::BinaryOperator::dumpTree(size_t indent) const
{
...
String PrometheusQueryTreeBase::AggregationOperator::dumpTree(size_t indent) const
{
...
template <typename TimeType, typename ScalarType>
String PrometheusQueryTreeBase::ConstInstantVector<TimeType, ScalarType>::dumpTree(size_t indent) const
{
    String str = makeIndent(indent) + "ConstInstantVector(labels = [";
    for (size_t i = 0; i != labels.size(); ++i)
        str += fmt::format("{}{} = {}", (i == 0 ? "" : ", "), labels[i].first, quoteString(labels[i].second));
    str += "]):";
    if (!point)
    {
        str += "empty\n";
    ...
    {
        str += "1 point:\n";
        str += fmt::format("{}time = {}, value = {}\n", makeIndent(indent + 1), timestampToString(point->first), scalarToString(point->second));
    }
    return str;
}

template <typename TimeType, typename ScalarType>
String PrometheusQueryTreeBase::ConstRangeVector<TimeType, ScalarType>::dumpTree(size_t indent) const
{
    String str = makeIndent(indent) + "ConstRangeVector(labels = [";
    for (size_t i = 0; i != labels.size(); ++i)
        str += fmt::format("{}{} = {}", (i == 0 ? "" : ", "), labels[i].first, quoteString(labels[i].second));
    str += "]): ";
    if (points.empty())
    ...
    else
    {
        ...
        for (const auto & [time, value] : points)
            str += fmt::format("{}time = {}, value = {}\n", makeIndent(indent + 1), timestampToString(time), scalarToString(value));
    }
    return str;
}
...
namespace
{
    ...
    template <typename TimeType>
    bool parseTimeRange(std::string_view input, TimeType & range, ErrorListener & error_listener, size_t input_pos)
    {
    ...
    template <typename TimeType>
    bool parseSubqueryRange(std::string_view input, TimeType & range, std::optional<TimeType> & resolution,
                            ErrorListener & error_listener, size_t input_pos)
    {
        /// Check opening and closing brackets.
        if (input.empty() || (input[0] != '['))
    ...
    class PromQLTreeBuilderBase : public antlr4_grammars::PromQLParserBaseVisitor
    {
    public:
        explicit PromQLTreeBuilderBase(std::string_view promql_query_, ErrorListener & error_listener_)
    ...
    private:
        ...
        using MatcherType = PrometheusQueryTreeBase::MatcherType;

        template <typename ScalarType>
        using ScalarLiteral = PrometheusQueryTreeBase::ScalarLiteral<ScalarType>;
        using StringLiteral = PrometheusQueryTreeBase::StringLiteral;
        using Matcher = PrometheusQueryTreeBase::Matcher;
        template <typename TimeType>
        using Selector = PrometheusQueryTreeBase::Selector<TimeType>;
        template <typename TimeType>
        using InstantSelector = PrometheusQueryTreeBase::InstantSelector<TimeType>;
        template <typename TimeType>
        using RangeSelector = PrometheusQueryTreeBase::RangeSelector<TimeType>;
        template <typename TimeType>
        using Subquery = PrometheusQueryTreeBase::Subquery<TimeType>;
        using Function = PrometheusQueryTreeBase::Function;
        using Operator = PrometheusQueryTreeBase::Operator;
        using UnaryOperator = PrometheusQueryTreeBase::UnaryOperator;
        using BinaryOperator = PrometheusQueryTreeBase::BinaryOperator;
        using AggregationOperator = PrometheusQueryTreeBase::AggregationOperator;
        template <typename TimeType, typename ScalarType>
        using ConstInstantVector = PrometheusQueryTreeBase::ConstInstantVector<TimeType, ScalarType>;
        template <typename TimeType, typename ScalarType>
        using ConstRangeVector = PrometheusQueryTreeBase::ConstRangeVector<TimeType, ScalarType>;

        /// Makes a node for a scalar literal after parsing it.
        virtual Node * makeScalarLiteral(antlr4::tree::TerminalNode * ctx) = 0;

        template <typename ScalarType>
        Node * makeScalarLiteralImpl(antlr4::tree::TerminalNode * ctx)
        {
            auto new_node = std::make_unique<ScalarLiteral<ScalarType>>();
        ...
        virtual Node * makeInstantSelector(antlr4_grammars::PromQLParser::InstantSelectorContext * ctx) = 0;

        template <typename TimeType>
        Node * makeInstantSelectorImpl(antlr4_grammars::PromQLParser::InstantSelectorContext * ctx)
        {
            auto new_node = std::make_unique<InstantSelector<TimeType>>();
            new_node->start_pos = getStartPos(ctx);
            new_node->length = getLength(ctx);
        ...
        virtual Node * makeRangeSelector(antlr4_grammars::PromQLParser::RangeSelectorContext * ctx) = 0;

        template <typename TimeType>
        Node * makeRangeSelectorImpl(antlr4_grammars::PromQLParser::RangeSelectorContext * ctx)
        {
            auto new_node = std::make_unique<RangeSelector<TimeType>>();
            new_node->start_pos = getStartPos(ctx);
            new_node->length = getLength(ctx);
        ...
        virtual bool addOffsetToNode(antlr4_grammars::PromQLParser::OffsetAtContext * ctx, Node & res_node) = 0;

        template <typename TimeType>
        bool addOffsetToNodeImpl(antlr4_grammars::PromQLParser::OffsetAtContext * ctx, Node & res_node)
        {
            std::optional<TimeType> at;
            TimeType offset;
            ...
            if (auto * instant_selector = typeid_cast<InstantSelector<TimeType> *>(&res_node))
            {
                instant_selector->at = at;
                instant_selector->offset = offset;
            }
            else if (auto * range_selector = typeid_cast<RangeSelector<TimeType> *>(&res_node))
            {
                range_selector->at = at;
                range_selector->offset = offset;
            }
            else if (auto * subquery = typeid_cast<Subquery<TimeType> *>(&res_node))
            {
                subquery->at = at;
        ...
        virtual Node * makeSubquery(antlr4_grammars::PromQLParser::SubqueryOpContext * ctx, Node * expression, size_t start_pos, size_t length) = 0;

        template <typename TimeType>
        Node * makeSubqueryImpl(antlr4_grammars::PromQLParser::SubqueryOpContext * ctx, Node * expression, size_t start_pos, size_t length)
        {
            auto new_node = std::make_unique<Subquery<TimeType>>();
    ...
    template <typename TimeType, typename ScalarType>
    class PromQLTreeBuilder : public PromQLTreeBuilderBase
    {
    private:
        using PromQLTreeBuilderBase::PromQLTreeBuilderBase;

        Node * makeScalarLiteral(antlr4::tree::TerminalNode * ctx) override
        {
            return makeScalarLiteralImpl<ScalarType>(ctx);
        }

        Node * makeInstantSelector(antlr4_grammars::PromQLParser::InstantSelectorContext * ctx) override
        {
            return makeInstantSelectorImpl<TimeType>(ctx);
        }

        Node * makeRangeSelector(antlr4_grammars::PromQLParser::RangeSelectorContext * ctx) override
        {
            return makeRangeSelectorImpl<TimeType>(ctx);
        }

        Node * makeSubquery(antlr4_grammars::PromQLParser::SubqueryOpContext * ctx, Node * expression, size_t start_pos, size_t length) override
        {
            return makeSubqueryImpl<TimeType>(ctx, expression, start_pos, length);
        }

        bool addOffsetToNode(antlr4_grammars::PromQLParser::OffsetAtContext * ctx, Node & res_node) override
        {
            return addOffsetToNodeImpl<TimeType>(ctx, res_node);
        }
    };
}
...
namespace
{
    /// Parses a prometheus query (helper function).
    template <typename TimeType, typename ScalarType>
    bool doParse(PrometheusQueryTree<TimeType, ScalarType> & res_, const String & promql_query_, bool throw_exception, String * error_message, size_t * error_pos)
    {
        #if USE_ANTLR4_GRAMMARS
            ...
            if (!error_listener.hasError())
            {
                PromQLTreeBuilder<TimeType, ScalarType> builder{promql_query_, error_listener};
            ...
            res_ = PrometheusQueryTree<TimeType, ScalarType>{promql_query_, parsed_root, std::move(parsed_nodes)};
            return true;
        #else
...
template <typename TimeType, typename ScalarType>
bool PrometheusQueryTree<TimeType, ScalarType>::tryParse(const String & promql_query_, String * error_message, size_t * error_pos)
{
    return doParse(*this, promql_query_, /* throw_exception = */ false, error_message, error_pos);
}

template <typename TimeType, typename ScalarType>
void PrometheusQueryTree<TimeType, ScalarType>::parse(const String & promql_query_)
{
    doParse(*this, promql_query_, /* throw_exception = */ true, /* error_message = */ nullptr, /* error_pos = */ nullptr);
}

template class PrometheusQueryTree<time_t, Float32>;
template class PrometheusQueryTree<time_t, Float64>;
template class PrometheusQueryTree<DecimalField<DateTime64>, Float32>;
template class PrometheusQueryTree<DecimalField<DateTime64>, Float64>;
