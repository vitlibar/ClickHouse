...
String PrometheusQueryTree::InstantSelector::dumpNode(const PrometheusQueryTree & /* tree */, size_t indent) const
{
    ...
    str += fmt::format("\n{}{} {}", makeIndent(indent + 1), matchers.size(), matchers_word);
    for (const auto & matcher : matchers)
        str += fmt::format("\n{}{} {} {})", makeIndent(indent + 2), matcher.label_name, matcher.matcher_type, quoteString(matcher.label_value));
    return str;
}
...
String PrometheusQueryTree::RangeSelector::dumpNode(const PrometheusQueryTree & tree, size_t indent) const
{
    ...
    str += fmt::format("\n{}instant_selector:\n{}", makeIndent(indent + 1), getInstantSelector()->dumpNode(tree, indent + 2));
    return str;
}
...
String PrometheusQueryTree::Subquery::dumpNode(const PrometheusQueryTree & tree, size_t indent) const
{
    ...
    str += fmt::format("\n{}expression:\n{}", makeIndent(indent + 1), getExpression()->dumpNode(tree, indent + 2));
    return str;
}
...
String PrometheusQueryTree::Function::dumpNode(const PrometheusQueryTree & tree, size_t indent) const
{
    ...
    str += fmt::format("\n{}{} {}", makeIndent(indent + 1), arguments.size(), arguments_word);
    for (const auto * argument : arguments)
        str += fmt::format("\n{}", argument->dumpNode(tree, indent + 2));
    return str;
}
...
String PrometheusQueryTree::UnaryOperator::dumpNode(const PrometheusQueryTree & tree, size_t indent) const
{
    ...
    str += fmt::format("\n{}argument:\n{}", makeIndent(indent + 1), getArgument()->dumpNode(tree, indent + 2));
    return str;
}
...
String PrometheusQueryTree::BinaryOperator::dumpNode(const PrometheusQueryTree & tree, size_t indent) const
{
    ...
    str += fmt::format("\n{}left_argument:\n{}", makeIndent(indent + 1), getLeftArgument()->dumpNode(tree, indent + 2));
    str += fmt::format("\n{}right_argument:\n{}", makeIndent(indent + 1), getRightArgument()->dumpNode(tree, indent + 2));
    return str;
}
