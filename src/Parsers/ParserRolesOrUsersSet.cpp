...
bool ParserRolesOrUsersSet::parseImpl(Pos & pos, ASTPtr & node, Expected & expected)
{
    ...
    if (!parseBeforeExcept(pos, expected, id_mode, allow_all, allow_current_user_tag, names, all, current_user))
        return false;

    parseExceptAndAfterExcept(pos, expected, id_mode, allow_current_user_tag, except_names, except_current_user);
