                                     ...
                                     String & cluster)
    {
        return IParserBase::wrapParseImpl(pos, [&]
        {
            ...
            if (allow_on_cluster)
            {
                ...
                if (!res_cluster.empty())
                    allow_on_cluster = false;
            }

            std::vector<std::pair<String, String>> database_and_table_names;
            if (allow_multiple_tables)
            {
            ...
            assert(database_and_table_names.size() >= 1);

            if (allow_on_cluster)
                parseOnCluster(pos, expected, res_cluster);

            std::vector<RowPolicy::NameParts> res_name_parts;
            res_name_parts.clear();
...
bool ParserRowPolicyNames::parseImpl(Pos & pos, ASTPtr & node, Expected & expected)
{
    ...
    auto parse_element = [&]
    {
        ...
        std::vector<RowPolicy::NameParts> name_parts_new;
        if (!parseRowPolicyNamesAroundON(pos, expected, name_parts.empty(), name_parts.empty(), allow_on_cluster, name_parts_new, cluster))
            return false;

        name_parts.insert(std::end(name_parts), std::make_move_iterator(name_parts_new.begin()), std::make_move_iterator(name_parts_new.end()));
        num_added_names_last_time = name_parts_new.size();
        return true;
    };
