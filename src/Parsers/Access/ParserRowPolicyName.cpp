...
namespace
{
    ...
    bool parseRowPolicyNamesAroundON(IParser::Pos & pos, Expected & expected,
                                     ...
                                     std::vector<RowPolicyName> & names,
                                     String & cluster)
    {
        return IParserBase::wrapParseImpl(pos, [&]
        {
            ...
            for (const String & short_name : short_names)
                for (const auto & [database, table_name] : database_and_table_names)
                    names.push_back({short_name, database, table_name});
...
bool ParserRowPolicyName::parseImpl(Pos & pos, ASTPtr & node, Expected & expected)
{
    std::vector<RowPolicyName> names;
    String cluster;
    if (!parseRowPolicyNamesAroundON(pos, expected, false, false, allow_on_cluster, names, cluster))
        return false;

    assert(names.size() == 1);
    auto result = std::make_shared<ASTRowPolicyName>();
    result->name = std::move(names.front());
...
bool ParserRowPolicyNames::parseImpl(Pos & pos, ASTPtr & node, Expected & expected)
{
    ...
    auto parse_around_on = [&]
    {
        ...
        std::vector<RowPolicyName> new_names;
        if (!parseRowPolicyNamesAroundON(pos, expected, names.empty(), names.empty(), allow_on_cluster, new_names, cluster))
            return false;

        num_added_names_last_time = new_names.size();
        boost::range::push_back(names, std::move(new_names));
        return true;
    };
