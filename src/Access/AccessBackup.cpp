...
AccessRestorerFromBackup::AccessRestorerFromBackup(
    ...
    , skip_unresolved_dependencies(restore_settings_.skip_unresolved_dependencies_of_access_entities)
    , update_dependants(restore_settings_.update_dependants_of_restored_access_entities)
...
void AccessRestorerFromBackup::addDataPath(const String & data_path_in_backup, bool dependants_only)
{
    if (loaded)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Access entities already loaded");
        
    if (dependants_only && !update_dependants)
        return;

    for (size_t i = 0; i != data_paths_in_backup.size(); ++i)
    {
        if (data_paths_in_backup[i].first == data_path_in_backup)
        {
            data_paths_in_backup[i].second &= dependants_only;
            return;
        }
    }

    data_paths_in_backup.emplace_back(data_path_in_backup, dependants_only);
}
...
void AccessRestorerFromBackup::loadFromBackup()
{
    ...
    for (size_t data_path_index = 0; data_path_index != data_paths_in_backup.size(); ++data_path_index)
    {
        ...
        bool dependants_only = data_paths_in_backup[data_path_index].second;
        ...
        for (const String & filename : filenames)
        {
            ...
            for (const auto & [id, entity] : ab.entities)
            {
                ...
                {
                    it = entity_infos.emplace(id, EntityInfo{.id = id, .name = entity->getName(), .type = entity->getType(), .entity = entity,
                                                             .data_path_index = data_path_index}).first;
                }
            ...
            for (const auto & [id, name_and_type] : ab.dependencies)
            {
                ...
                if (it == entity_infos.end())
                {
                    it = entity_infos.emplace(id, EntityInfo{.id = id, .name = name_and_type.first, .type = name_and_type.second,
                                                             .data_path_index = data_path_index}).first;
                }
                EntityInfo & entity_info = it->second;
...
void AccessRestorerFromBackup::generateRandomIDsAndResolveDependencies(const AccessControl & access_control)
{
    ...
    /// Calculate `is_dependant` for each entity info.
    if (update_dependants)
    {
        for (auto & [id, entity_info] : entity_infos)
        {
            if (!entity_info.restore && entity_info.new_id && entity_info.entity && entity_info.entity->hasDependencies(old_ids_to_restore))
                entity_info.is_dependant = true;
        }
    }

    /// Remap the UUIDs of dependencies in the access entities we're goind to restore.
    for (auto & [id, entity_info] : entity_infos)
    {
        if (entity_info.entity && (entity_info.restore || entity_info.is_dependant))
        {
...
std::vector<std::pair<UUID, AccessEntityPtr>> AccessRestorerFromBackup::getDependants(const String & data_path_in_backup) const
{
    if (!ids_assigned)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "IDs not assigned");
    ...
    const RestoreSettings & restore_settings)
{
    ...
    bool update_dependants = restore_settings.update_dependants_of_restored_access_entities;

    std::unordered_map<UUID, UUID> new_to_existing_ids;
    std::vector<std::pair<UUID, AccessEntityPtr>> more_dependants;
    ...
    for (const auto & [id, entity] : entities)
    {
        ...
        else
        {
            ...
            if (update_dependants)
                more_dependants.emplace_back(existing_id, entity);
        }
    }
    ...
    auto do_update_dependants = [&](const std::vector<std::pair<UUID, AccessEntityPtr>> & dependants_param)
    {
        if (dependants_param.empty())
            return;

        ids_to_update.clear();
        ids_to_update.reserve(dependants_param.size());
        std::unordered_map<UUID, AccessEntityPtr> id_to_dependant;

        for (const auto & [id, entity] : dependants_param)
        {
        ...
        {
            const auto & src_entity = *id_to_dependant.at(id);
            if (!src_entity.hasDependencies(restored_ids))
                return entity;
            LOG_TRACE(log, "{}: Updating dependant", entity->formatTypeWithName());
            auto res = entity->clone();
    ...
    do_update_dependants(dependants);
    do_update_dependants(more_dependants);
}
