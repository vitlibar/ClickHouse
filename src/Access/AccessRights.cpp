...
namespace
{
    struct ProtoElement
    {
        ...
        AccessRightsElement getResult() const
        {
            ...
            res.is_partial_revoke = is_revoke;
    ...
    private:
        ...
        void appendResultWithElementsWithDifferenceInColumnOnly(size_t start, size_t count, AccessRightsElements & res) const
        {
            ...
            while (pbegin < pend)
            {
                ...
                back.is_partial_revoke = pbegin->is_revoke;
...
void AccessRights::grant(const AccessRightsElement & element)
{
    if (element.is_partial_revoke)
        throw Exception("A partial revoke must be revoked, not granted", ErrorCodes::BAD_ARGUMENTS);
...
void AccessRights::grantWithGrantOption(const AccessRightsElement & element)
{
    if (element.is_partial_revoke)
        throw Exception("A partial revoke must be revoked, not granted", ErrorCodes::BAD_ARGUMENTS);
    grantImpl<true>(element);
...
void AccessRights::grantWithGrantOption(const AccessRightsElements & elements)
{
    for (const auto & element : elements)
        grantWithGrantOption(element);
}
...
void AccessRights::revokeGrantOption(const AccessRightsElement & element)
{
    revokeImpl<true>(element);
}
...
void AccessRights::revokeGrantOption(const AccessRightsElements & elements)
{
    for (const auto & element : elements)
        revokeGrantOption(element);
}
...
{
    if (element.is_partial_revoke)
        throw Exception("Cannot check if granted for partial revoke", ErrorCodes::BAD_ARGUMENTS);
    if (element.grant_option)
...
bool AccessRights::hasGrantOption(const AccessRightsElement & element) const
{
    if (element.is_partial_revoke)
        throw Exception("Cannot check if granted for partial revoke", ErrorCodes::BAD_ARGUMENTS);
    return isGrantedImpl<true>(element);
...
bool AccessRights::hasGrantOption(const AccessRightsElements & elements) const
{
    for (const auto & element : elements)
        if (!hasGrantOption(element))
            return false;
    return true;
}
