...
namespace
{
    ...
    std::vector<AccessEntityPtr> parseRowPolicies(const Poco::Util::AbstractConfiguration & config, bool no_permissive_policies_no_rows)
    {
        ...
        for (auto & [database_and_table_name, user_to_filters] : all_filters_map)
        {
            ...
            for (const String & user_name : user_names)
            {
                ...
                {
                    if (no_permissive_policies_no_rows)
                        filter = "1";
                    else
                        continue;
                }
...
void UsersConfigAccessStorage::parseFromConfig(const Poco::Util::AbstractConfiguration & config)
{
    try
    {
        ...
        for (const auto & entity : parseQuotas(config))
            all_entities.emplace_back(generateID(*entity), entity);
        for (const auto & entity : parseRowPolicies(config, access_control.isEnabledNoPermissivePoliciesNoRows()))
            all_entities.emplace_back(generateID(*entity), entity);
