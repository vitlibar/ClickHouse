...
namespace
{
        ...
        for (const String & setting_name : keys)
        {
            ...
            for (const String & constraint_type : constraint_types)
            {
                if (constraint_type == "min")
                    profile_element.min_value = Settings::valueToCorrespondingType(setting_name, config.getString(path_to_name + "." + constraint_type));
                else if (constraint_type == "max")
                    profile_element.max_value = Settings::valueToCorrespondingType(setting_name, config.getString(path_to_name + "." + constraint_type));
        ...
        const String & profile_name)
    {
        ...
        for (const std::string & key : keys)
        {
            ...
            SettingsProfileElement profile_element;
            profile_element.setting_name = setting_name;
            profile_element.value = Settings::valueToCorrespondingType(setting_name, config.getString(profile_config + "." + key));
            profile->elements.emplace_back(std::move(profile_element));
        }
