...
namespace
{
    ...
    public:
        ...
        ASTPtr getResult(bool no_permissive_policies_no_rows) &&
        {
            if (!permissions.empty() || no_permissive_policies_no_rows)
            {
...
void RowPolicyCache::mixFiltersFor(EnabledRowPolicies & enabled)
{
    ...
    for (auto & [key, mixer] : mixers)
    {
        ...
        mixed_filter.ast = std::move(mixer.mixer).getResult(access_control.isEnabledNoPermissivePoliciesNoRows());
    }
