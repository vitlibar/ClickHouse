...
IndexOfBlockForNativeFormat IndexOfBlockForNativeFormat::getColumns(const NameSet & required_columns) const
{
    ...
    {
        const auto & column = columns[i];
        if (required_columns.count(column.name))
...
IndexForNativeFormat IndexForNativeFormat::getColumns(const NameSet & required_columns) const
{
    IndexForNativeFormat res;
    res.blocks.reserve(blocks.size());
    for (const auto & block : blocks)
        res.blocks.emplace_back(block.getColumns(required_columns));
    return res;
}
