...
StorageTimeSeriesSelector::Configuration StorageTimeSeriesSelector::getConfiguration(ASTs & args, const ContextPtr & context)
{
    ...
    auto value_data_type = data_table_metadata->columns.get(TimeSeriesColumnNames::Value).type;
    ...
    config.value_data_type = std::move(value_data_type);
    ...
    size_t /* num_streams */)
{
    ...
    ASTPtr select_query_from_tags_table = makeSelectQueryFromTagsTable(tags_table_id, matchers, column_name_by_tag_name, min_time_to_filter_ids, max_time_to_filter_ids, config.timestamp_data_type);
    ASTPtr select_query_from_data_table = makeSelectQueryFromDataTable(data_table_id, select_query_from_tags_table, config.min_time, config.max_time, config.timestamp_data_type);
