...
StorageTimeSeriesSelector::Configuration StorageTimeSeriesSelector::getConfiguration(ASTs & args, const ContextPtr & context)
{
    ...
    if (isDateTime64(timestamp_type))
        timestamp_scale = getDecimalScale(*timestamp_type);
...
namespace
{
                                        ...
                                        const DataTypePtr & timestamp_type)
    {
        ...
        {
            auto where_filter = makeWhereFilterForDataTable(select_query_from_tags_table, min_time, max_time, timestamp_type);
            select_query->setExpression(ASTSelectQuery::Expression::WHERE, std::move(where_filter));
        }
    ...
    size_t /* num_streams */)
{
    ...
    ASTPtr select_query_from_tags_table = makeSelectQueryFromTagsTable(tags_table_id, matchers, column_name_by_tag_name, min_time_to_filter_ids, max_time_to_filter_ids, config.timestamp_type);
    ASTPtr select_query_from_data_table = makeSelectQueryFromDataTable(data_table_id, select_query_from_tags_table, config.min_time, config.max_time, config.timestamp_type);
