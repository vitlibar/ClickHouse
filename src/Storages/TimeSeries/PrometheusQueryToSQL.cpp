...
public:
    explicit ASTBuilder(const PrometheusQueryToSQLConverter & converter_)
        ...
        , value_data_type(getEvaluationSettings().value_data_type)
        , result_type(converter_.result_type)
    {
        ...
        else
        {
            ...
            lookback_delta = 5 * 60 * DecimalUtils::scaleMultiplier<Int64>(timestamp_scale);
        }
        ...
        else
        {
            ...
            default_resolution = 15 * DecimalUtils::scaleMultiplier<Int64>(timestamp_scale);
        }
    }
...
private:
    ...
    Piece buildPieceForInstantSelector(const PrometheusQueryTree::InstantSelector * instant_selector) const
    {
        ...
        res.from_table_function = makeASTFunction("timeSeriesSelector",
            std::make_shared<ASTLiteral>(getTimeSeriesStorageID().getDatabaseName()),
            std::make_shared<ASTLiteral>(getTimeSeriesStorageID().getTableName()),
...
ColumnsDescription PrometheusQueryToSQLConverter::getResultColumns() const
{
    ...
    switch (result_type)
    {
        ...
        {
            columns.add(ColumnDescription{TimeSeriesColumnNames::Scalar, evaluation_settings.value_data_type});
            break;
        }
        ...
        case ResultType::INSTANT_VECTOR:
        {
            ...
            columns.add(
                ColumnDescription{
                    ...
                    evaluation_settings.value_data_type});
            break;
        }
        case ResultType::RANGE_VECTOR:
        {
            ...
            columns.add(
                ColumnDescription{
                    ...
                    std::make_shared<DataTypeArray>(std::make_shared<DataTypeTuple>(
                        DataTypes{evaluation_settings.timestamp_data_type, evaluation_settings.value_data_type}))});
            break;
        }
    }
    return columns;
