...
namespace
{
    ...
    T fieldToDecimal(const Field & field, UInt32 target_scale)
    {
        switch (field.getType())
        {
            case Field::Types::Int64:
                return field.safeGet<Int64>() * DecimalUtils::scaleMultiplier<Int64>(target_scale);
            case Field::Types::UInt64:
                return field.safeGet<UInt64>() * DecimalUtils::scaleMultiplier<Int64>(target_scale);
            case Field::Types::Float64:
                return static_cast<Int64>(field.safeGet<Float64>() * DecimalUtils::scaleMultiplier<Int64>(target_scale));
    ...
    ASTPtr decimalToAST(T decimal, UInt32 scale, const DataTypePtr & data_type)
    {
        ...
        if (data_type_idx == TypeIndex::DateTime64)
        {
            ...
            return makeASTFunction("toDecimal64", std::make_shared<ASTLiteral>(toString(decimal, scale)), std::make_shared<ASTLiteral>(scale));
        else
            return std::make_shared<ASTLiteral>(DecimalField<T>{decimal, scale});
    }
