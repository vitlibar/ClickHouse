...
private:
    ...
    Field nodeToField(const PrometheusQueryTree::Node * node) const
    {
        auto node_type = node->node_type;
        if (node_type == NodeType::Scalar)
            return Field{typeid_cast<const PrometheusQueryTree::Scalar &>(*node).scalar};
        else if (node_type == NodeType::Timestamp)
            return DecimalField<DateTime64>{typeid_cast<const PrometheusQueryTree::Timestamp &>(*node).timestamp, getPromQLTree().getTimestampScale()};
        else if (node_type == NodeType::Duration)
            return DecimalField<Decimal64>{typeid_cast<const PrometheusQueryTree::Duration &>(*node).duration, getPromQLTree().getTimestampScale()};
        else
            throw Exception(ErrorCodes::BAD_ARGUMENTS, "Expected a scalar literal or a duration node, got {} ({})", node_type, getPromQLText(node));
    }
