    ...
    const PQT::BinaryOperator * operator_node, SQLQueryPiece && left_argument, SQLQueryPiece && right_argument, ConverterContext & context)
{
    ...
    /// Step 2:
    /// SELECT group,
    ///        if(notEmpty(join_counts), arrayMap(x, y -> if(y = 0, x, NULL), values, join_counts), values) AS values
    ...
    ASTPtr step2;
    {
        ...
        builder.join_on = makeASTFunction(
            "equals",
            makeExpressionForJoinGroup(operator_node, make_intrusive<ASTIdentifier>(ColumnNames::Group), left_argument.metric_name_dropped),
