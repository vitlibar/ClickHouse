...
namespace
{
        ...
        std::function<ASTPtr(ASTPtr, ASTPtr)> apply_function_to_ast)
    {
        ...
        for (auto side : std::vector<std::string_view>{left, right})
        {
        ...
        /// FROM left SEMI LEFT JOIN right
        ...
        bool metric_name_dropped_from_result = false;
        {
            ...
            JoinKind join_kind = JoinKind::Left;
            ...
            if (!group_left && !group_right)
            {
                ...
                /// We look for matches which as we know are unique on both sides.
            }
            else
            {
