...
namespace
{
    /// Applies a basic binary operator to operands if at least one of them is scalar.
    ...
    /// Applied a basic operator if both operands are instant vectors.
        ...
        BinaryOperatorNoGroupingTagsSource no_grouping_tags_source)
    {
        ...
        bool metric_name_dropped_from_result = false;
        {
            ...
            if (!group_left && !group_right)
            {
                ...
                else if (no_grouping_tags_source == BinaryOperatorNoGroupingTagsSource::Right)
                {
                    new_group = make_intrusive<ASTIdentifier>(Strings{right, ColumnNames::OriginalGroup});
                    metric_name_dropped_from_result = right_argument.metric_name_dropped;
                }
                else
                {
