...
ASTs getToGridAggregateFunctionArguments(const SQLQueryPiece & range_vector, ConverterContext & context)
{
    ...
    switch (range_vector.store_method)
    {
        ...
        {
            /// timestamps: (timeSeriesFromGrid(<start_time>, <end_time>, <step>, values) AS time_series).1
            /// values:     time_series.2
            ASTPtr time_series = makeASTFunction(
                "timeSeriesFromGrid",
            ...
            timestamps = makeASTFunction("tupleElement", std::move(time_series), make_intrusive<ASTLiteral>(1));
            values = makeASTFunction(
