...
namespace
{
    ...
    /// Applied a simple operator if both operands are instant vectors.
        ...
        bool drop_metric_name)
    {
        ...
        bool metric_name_dropped_from_result = false;
        {
            ...
            if (!group_left && !group_right)
            {
                ...
                {
                    /// With on(tags), the join_group already contains exactly the tags we want to use as the result group.
                ...
                {
                    /// With ignoring(tags), the join_group has both the ignored tags and the metric name removed, but
                    /// since `drop_metric_name == false` (comparison operators without bool modifier) the result should keep the metric name.
                    /// So we recompute from original_group, removing only the ignored tags (drop_metric_name=false keeps __name__).
                ...
                {
                    /// The metric name may be already dropped from the join group, however
                    /// it also may be not dropped if it's specified explicitly in on(), for example a + on(__name__) b
