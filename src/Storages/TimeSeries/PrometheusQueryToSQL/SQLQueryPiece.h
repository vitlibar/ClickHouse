...
enum class StoreMethod
{
    ...
    /// Can be used with ResultType::SCALAR only.
    /// CONST_SCALAR is produced by a float literal in a prometheus query.
    CONST_SCALAR,

    /// A const string value stored in `QueryPiece::string_value`.
    /// Can be used with ResultType::STRING only.
    /// CONST_STRING is produced by a string literal in a prometheus query.
    CONST_STRING,

    /// Data are stored in three columns `timestamp` (timestamp_type), 'value` (floating point), `group` (UInt64).
    /// The order of rows is not specified.
    /// Can be used with ResultType::RANGE_VECTOR only.
    /// RAW_DATA is produced by using selectors in a prometheus query.
    RAW_DATA,

    /// Data are stored in two columns `values` (array of nullable floating-point values) and `group` (UInt64).
    /// Values of each row are aligned to the same time grid. Each value of `group` can appear only once in the output.
    /// The order of rows is not specified unless a function like sort_by_label() or sort_by_value() is used.
...
{
    SQLQueryPiece(const PrometheusQueryTree::Node * promql_node_, ResultType type_, StoreMethod store_method_)
        : promql_node(promql_node_), type(type_), store_method(store_method_) {}
