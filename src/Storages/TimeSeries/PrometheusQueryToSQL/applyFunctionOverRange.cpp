    ...
    ConverterContext & context)
{
    ...
    if (start_time > end_time)
    {
        ...
        /// SELECT arrayJoin(emptyArrayUInt64()) AS group,
        ///        defaultValueOfTypeName(Array(Nullable(Float64))) AS values
        ...
        params.select_list.push_back(makeASTFunction("defaultValueOfTypeName", std::make_shared<ASTLiteral>("Array(Nullable(Float64))")));
        params.select_list.back()->setAlias(TimeSeriesColumnNames::Values);

        SQLQueryPiece res{promql_node, ResultType::INSTANT_VECTOR, StoreMethod::VECTOR_GRID};
    ...
    switch (argument.store_method)
    {
        case StoreMethod::CONST_SCALAR:
        {
            ...
            ///                             replicate(number_of_steps, [scalar_value])
            ...
            SQLQueryPiece res{promql_node, ResultType::INSTANT_VECTOR, StoreMethod::SCALAR_GRID};
        ...
        {
            /// SELECT group,
            ///        <aggregate_function>(timeSeriesRange(start_time, end_time, step),
            ///                             values)
            /// GROUP BY group
            SelectQueryParams params;
            ...
            SQLQueryPiece res{promql_node, ResultType::INSTANT_VECTOR, argument.store_method};
