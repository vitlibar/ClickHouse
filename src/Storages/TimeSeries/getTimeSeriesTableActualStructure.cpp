    ...
    for (const auto & column : src)
    {
        if (!res.has(column.name))
        {
            throw Exception(
                ...
                "Table {}.{} can't find a place for column {}. "
                "The TimeSeries table engine support only a limited set of columns (id, timestamp, value, metric_name, tags, metric_family_name, type, unit, help). "
                "Extra columns representing tags in the tags table must be specified in the 'tags_to_columns' setting. "
                "Other extra columns must be specified in the 'extra_columns' setting",
                create_query.getDatabase(),
                create_query.getTable(),
                column.name);
        }
    }
