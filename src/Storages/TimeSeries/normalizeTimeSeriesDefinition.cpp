...
namespace
{
    ...
    /// Cross-source conflict-checking setter for `DataTypePtr`.
    /// The first non-null value wins; any subsequent non-null value must equal it.
    void setOrCheckDataType(
        DataTypePtr & target, String & target_source,
        const DataTypePtr & value, const String & value_source,
        std::string_view what, const StorageID & table_id_for_error)
    {
        if (!value)
            return;
        if (target)
        {
            if (!target->equals(*value))
                throw Exception(ErrorCodes::BAD_TYPE_OF_FIELD,
                    ...
                    table_id_for_error.getNameForLogs(), what,
    ...
    /// Reads tags INNER COLUMNS declarations and extracts the id type.
    ...
    /// Reads types from columns of external `TO` target tables referenced by a CREATE query.
    void readTypesFromExternalTargets(
        const ASTCreateQuery & query, const ContextPtr & context,
        DataTypePtr & timestamp_type, String & timestamp_src,
        DataTypePtr & scalar_type, String & scalar_src,
        DataTypePtr & id_type, String & id_src,
        const StorageID & table_id_for_error)
    {
        ...
        if (!samples_id.empty())
            readTypesFromExternalSamples(samples_id, samples_columns,
                timestamp_type, timestamp_src, scalar_type, scalar_src, id_type, id_src, table_id_for_error);

        auto [tags_id, tags_columns] = resolve_external(ViewTarget::Tags);
        if (!tags_id.empty())
            readTypesFromExternalTags(tags_id, tags_columns,
                id_type, id_src, table_id_for_error);
    }

    /// Cross-source-resolved column types needed during normalization.
    ...
    /// Resolves timestamp / scalar / id types from all explicit sources (with cross-source agreement),
    /// sets by defaults the types which are not set explicitly.
            ...
            case ViewTarget::Tags:
            {
                ...
                add_column_if_missing(TimeSeriesColumnNames::ID, dataTypeToAST(resolved.id_type));
                {
                        ...
                        && !time_series_settings[TimeSeriesSetting::id_generator].value)
                    {
                        ...
                        new_decl.setDefaultExpression(makeASTForTimeSeriesIDGenerator(resolved.id_type, time_series_settings, table_id_for_error));
                        changed = true;
                    }
                }
    ...
    /// New-version CREATE queries always persist `INNER COLUMNS` for every inner target,
    /// so the absence of one is the reliable signal that the query came from an older format.
    bool isOldVersion(const ASTCreateQuery & create_query)
    {
        for (auto kind : getTargetKinds())
        {
            if (!create_query.hasTargetTableID(kind) && !create_query.getTargetInnerColumns(kind))
                return true;
        }
        return false;
    }

    /// Generates inner column lists.
    /// Used for upgrading from old format where inner columns weren't stored in the query.
    /// Returns true if any inner column list was generated.
    bool generateInnerColumnsForOldVersion(
        ASTCreateQuery & create_query,
        const TimeSeriesSettings & time_series_settings)
    {
        StorageID table_id_for_error{create_query.getDatabase(), create_query.getTable()};

        /// Map of `name -> ASTColumnDeclaration` from the create query's outer column list.
        ...
        /// Old-format queries persisted flat outer columns (`id`, `timestamp`, `value`, ...).
        ...
        for (auto inner_table_kind : getTargetKinds())
        {
            ...
            switch (inner_table_kind)
            {
                case ViewTarget::Samples:
                {
                    /// Column "id" - no DEFAULT in the samples table: the identifier is computed in the "tags" inner
                    /// table because it depends on columns like "metric_name" or "all_tags" which don't exist in samples.
                    /// Reset any DEFAULT carried over from an `id` column on the outer column list.
                ...
                {
                    /// Column "id" - with a DEFAULT expression that computes the identifier from "metric_name" and tags.
                    /// The DEFAULT is derived from the id type only when the `id_generator` setting is not set —
                    /// runtime precedence is setting → column DEFAULT → derived, so when the setting is set the
                    /// column DEFAULT is redundant and would also go stale after `ALTER ... MODIFY SETTING id_generator`.
                    add_column(TimeSeriesColumnNames::ID, dataTypeToAST(id_type));
                    ...
                    if (!time_series_settings[TimeSeriesSetting::id_generator].value)
                    {
                        ...
                        new_decl.default_specifier = ColumnDefaultSpecifier::Default;
                        new_decl.ephemeral_default = false;
                        new_decl.setDefaultExpression(makeASTForTimeSeriesIDGenerator(id_type, time_series_settings, table_id_for_error));
                    }
    ...
    /// Builds a sensible default engine definition for an inner table.
    ...
    /// looks up the CREATE query of <other_table> and applies it to `create_query`
    /// outer columns, inner columns, inner engines and settings.
    /// Returns true if anything was changed in `create_query`.
    bool applyASClause(ASTCreateQuery & create_query, const ContextPtr & context)
    {
        ...
        /// Apply inner columns and inner engines.
        for (auto kind : getTargetKinds())
        {
...
bool normalizeTimeSeriesDefinition(ASTCreateQuery & create_query, const ContextPtr & context, LoadingStrictnessLevel mode, bool is_restore_from_backup)
{
    ...
    /// Older versions didn't store inner table column definitions in the `CREATE` query — reconstruct them now.
    if (!is_new_table && isOldVersion(create_query))
    {
        TimeSeriesSettings settings_for_old_version;
        if (create_query.storage)
            settings_for_old_version.loadFromQuery(*create_query.storage);
        changed |= generateInnerColumnsForOldVersion(create_query, settings_for_old_version);
    }

    /// Look up the AS-source (if any) and apply its `AS` clause to `create_query` — merges settings,
    /// column lists, and inner engine into `create_query`.
    if (!create_query.as_table.empty())
        changed |= applyASClause(create_query, context);

    TimeSeriesSettings settings;
    if (create_query.storage)
        settings.loadFromQuery(*create_query.storage);

    /// Resolve the three column types (timestamp, scalar, id) from the various sources.
    /// Verify that all explicit declarations agree, and apply defaults.
    ResolvedTimeSeriesTypes resolved = resolveTimeSeriesTypes(create_query, context);
    ...
    if (is_new_table)
    {
        for (auto kind : getTargetKinds())
        {
            ...
            else
            {
                ...
                if (normalizeInnerTableColumns(*inner_columns, kind, settings, resolved,
                        StorageID{create_query.getDatabase(), create_query.getTable()}))
                {
