...
namespace
{
        ...
        const StorageID & table_id)
    {
        ...
        for (const auto & column : query.columns_list->columns->children)
        {
            ...
            /// Flat `id`/`timestamp`/`value` columns belong to the prealpha shape and must not appear
            /// here — on ATTACH the prealpha migration has already removed them, on CREATE the user
            /// should use INNER COLUMNS instead.
    ...
    /// on ATTACH externals may not be loaded yet, so we skip reading from them.
    ...
    /// Detects prealpha version by outer columns: prealpha had `id`, `timestamp`, `value` outer columns;
    ...
    /// Migrates a prealpha-format CREATE query: generates `INNER COLUMNS` for inner targets and
    /// replaces flat outer columns with a single `time_series` column carrying the resolved types.
    /// `normalizeTimeSeriesDefinition` rebuilds the canonical full outer column list afterwards.
    bool upgradeFromPrealpha(
        ...
        const TimeSeriesSettings & time_series_settings)
    {
        ...
        /// The prealpha version persisted flat outer columns (`id`, `timestamp`, `value`, ...).
    ...
    /// outer columns, inner columns, inner engines, and the `SETTINGS` clause.
...
bool normalizeTimeSeriesDefinition(ASTCreateQuery & create_query, const ContextPtr & context, LoadingStrictnessLevel mode, bool is_restore_from_backup)
{
    ...
    /// Migrate prealpha-format CREATE queries to the current shape.
    if (!is_new_table && isPrealpha(create_query))
    {
        ...
        changed |= upgradeFromPrealpha(create_query, settings_for_prealpha);
    }
    ...
    ResolvedTimeSeriesTypes resolved = resolveTimeSeriesTypes(create_query, context, /*check_external_targets=*/ is_new_table);
