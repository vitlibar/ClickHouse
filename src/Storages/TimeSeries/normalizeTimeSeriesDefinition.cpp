...
bool normalizeTimeSeriesSettings(TimeSeriesSettings & settings, const ASTCreateQuery & create_query, const ContextPtr & context)
{
    ...
    {
        /// If id_generator wasn't set, then we can be updating from an older version.
...
bool normalizeTimeSeriesColumns(ColumnsDescription & columns, const TimeSeriesSettings & settings)
{
    ...
    auto skip_column = [&](const String & name)
    {
        original_column_names.erase(name);
        changed = true;
    };
    ...
    {
        if (id_column.default_desc != ColumnDefault{})
        {
    ...
    if (!move_original_column(TimeSeriesColumnNames::Type))
        add_column(ColumnDescription{TimeSeriesColumnNames::Type, std::make_shared<DataTypeLowCardinality>(std::make_shared<DataTypeString>())});
    if (!move_original_column(TimeSeriesColumnNames::Unit))
        add_column(ColumnDescription{TimeSeriesColumnNames::Unit, std::make_shared<DataTypeLowCardinality>(std::make_shared<DataTypeString>())});
