...
namespace
{
        ...
        const TimeSeriesSettings & time_series_settings)
    {
        ...
        if (inner_table_columns.columns)
            inner_table_columns.columns->children = std::move(new_list->children);
        else
        {
            inner_table_columns.columns = new_list.get();
            inner_table_columns.children.push_back(std::move(new_list));
        }
        ...
        const TimeSeriesSettings & time_series_settings)
    {
        ...
        switch (inner_table_kind)
        {
            ...
            case ViewTarget::Metrics:
            {
                ...
                add_column(TimeSeriesColumnNames::Type, makeASTDataType("LowCardinality", makeASTDataType("String")));
                add_column(TimeSeriesColumnNames::Unit, makeASTDataType("LowCardinality", makeASTDataType("String")));
...
ColumnsDescription generateTimeSeriesColumns(const TimeSeriesSettings & normalized_settings)
{
    ...
    add_column(TimeSeriesColumnNames::MetricName, DataTypeFactory::instance().get(makeASTDataType("LowCardinality", makeASTDataType("String"))));
    ...
    for (const auto & tag_name_and_column_name : tags_to_columns)
    {
        const auto & tuple = tag_name_and_column_name.safeGet<Tuple>();
        const auto & column_name = tuple.at(1).safeGet<String>();
        add_column(column_name, DataTypeFactory::instance().get(makeASTDataType("String")));
    }
    ...
    add_column(TimeSeriesColumnNames::Tags,
        DataTypeFactory::instance().get(makeASTDataType("Map", makeASTDataType("LowCardinality", makeASTDataType("String")), makeASTDataType("String"))));
    ...
    add_column(TimeSeriesColumnNames::AllTags,
        DataTypeFactory::instance().get(makeASTDataType("Map", makeASTDataType("String"), makeASTDataType("String"))));
    ...
    add_column(TimeSeriesColumnNames::MetricFamilyName, DataTypeFactory::instance().get(makeASTDataType("String")));
    add_column(TimeSeriesColumnNames::Type, DataTypeFactory::instance().get(makeASTDataType("LowCardinality", makeASTDataType("String"))));
    add_column(TimeSeriesColumnNames::Unit, DataTypeFactory::instance().get(makeASTDataType("LowCardinality", makeASTDataType("String"))));
    add_column(TimeSeriesColumnNames::Help, DataTypeFactory::instance().get(makeASTDataType("String")));
    ...
    for (auto kind : getTargetKinds())
    {
        if (create_query.hasTargetTableID(kind))
        {
            ...
            if (is_new_table)
            {
                ...
                checkTimeSeriesTargetTable(target_table_id, target_metadata->columns, kind, settings);
            }
        }
        else
        {
