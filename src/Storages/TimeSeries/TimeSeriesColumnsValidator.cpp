...
void TimeSeriesColumnsValidator::validateColumns(const ColumnsDescription & columns, const TimeSeriesSettings & time_series_settings) const
{
    auto get_column_description = [&](const String & column_name) -> const ColumnDescription &
    {
        ...
        if (!column)
        {
            throw Exception(ErrorCodes::THERE_IS_NO_COLUMN, "{}: Column {} is required for the TimeSeries table engine",
    ...
    {
        validateColumnForTimestamp(get_column_description(TimeSeriesColumnNames::MinTime));
        validateColumnForTimestamp(get_column_description(TimeSeriesColumnNames::MaxTime));
    }

    validateColumnForTimestamp(get_column_description(TimeSeriesColumnNames::MetricFamilyName));
    validateColumnForTimestamp(get_column_description(TimeSeriesColumnNames::Type));
    validateColumnForTimestamp(get_column_description(TimeSeriesColumnNames::Unit));
    validateColumnForTimestamp(get_column_description(TimeSeriesColumnNames::Help));
}
