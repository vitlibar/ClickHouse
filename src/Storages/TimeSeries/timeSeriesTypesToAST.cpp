...
ASTPtr timeSeriesTimestampToAST(DateTime64 timestamp, const DataTypePtr & timestamp_data_type)
{
    if (isDateTime64(timestamp_data_type))
    {
        ...
        if (str.find_first_of(".eE") == String::npos)
            str += ".";
        return makeASTCastTimeSeriesTimestamp(make_intrusive<ASTLiteral>(std::move(str)), timestamp_data_type);
    }
    ...
    {
        return makeASTCastTimeSeriesTimestamp(make_intrusive<ASTLiteral>(timestamp.value), timestamp_data_type);
    }
}
...
ASTPtr timeSeriesDurationToAST(Decimal64 duration, const DataTypePtr & timestamp_data_type)
{
    if (isDateTime64(timestamp_data_type) || isDecimal(timestamp_data_type))
    {
        ...
        return makeASTCastTimeSeriesDuration(make_intrusive<ASTLiteral>(std::move(str)), timestamp_data_type);
    }
    else
    {
        return makeASTCastTimeSeriesDuration(make_intrusive<ASTLiteral>(duration.value), timestamp_data_type);
    }
}
...
{
    return makeASTCastTimeSeriesScalar(make_intrusive<ASTLiteral>(value), scalar_data_type);
}


ASTPtr makeASTCastTimeSeriesTimestamp(ASTPtr && ast, const DataTypePtr & timestamp_data_type)
{
...
ASTPtr makeASTCastTimeSeriesDuration(ASTPtr && ast, const DataTypePtr & timestamp_data_type)
{
    WhichDataType which_data_type{timestamp_data_type};
    if (which_data_type.isDateTime64())
    {
...
ASTPtr makeASTCastTimeSeriesScalar(ASTPtr && ast, const DataTypePtr & scalar_data_type)
{
