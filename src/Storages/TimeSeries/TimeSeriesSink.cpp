...
namespace
{
    /// Collects tags from the input tags map for a given row, adds __name__ from the metric_name column if provided,
    /// sorts by name, removes exact duplicates and tags with empty values.
    void sortTagsAndRemoveDuplicates(
        const IColumn & tag_keys,
        const IColumn & tag_values,
        const ColumnArray::Offsets & tag_offsets,
        const IColumn * metric_name_col,
        size_t row,
        std::vector<std::pair<std::string_view, std::string_view>> & res)
    {
        size_t start = (row == 0) ? 0 : tag_offsets[row - 1];
        size_t end = tag_offsets[row];

        res.clear();
        res.reserve(end - start + 1);

        for (size_t j = start; j < end; ++j)
            res.emplace_back(tag_keys.getDataAt(j), tag_values.getDataAt(j));

        if (metric_name_col)
        {
            std::string_view metric_name_sv = metric_name_col->getDataAt(row);
            if (!metric_name_sv.empty())
                res.emplace_back(TimeSeriesTagNames::MetricName, metric_name_sv);
        }

        std::sort(res.begin(), res.end());
        res.erase(std::unique(res.begin(), res.end()), res.end());

        auto adjacent = std::adjacent_find(res.begin(), res.end(),
        ...
        auto it = std::lower_bound(res.begin(), res.end(), TimeSeriesTagNames::MetricName,
            [](const auto & tag, const char * name) { return tag.first < name; });
        if (it == res.end() || it->first != TimeSeriesTagNames::MetricName)
    ...
    , async_insert(async_insert_)
{
    ...
    /// If insert_columns is empty (e.g. INSERT INTO ts VALUES ...), all columns are being inserted.
    ...
    insert_tags_and_samples = is_insert_column(TimeSeriesColumnNames::TimeSeries)
        || is_insert_column(TimeSeriesColumnNames::MetricName)
...
void TimeSeriesSink::initTagsAndSamplesPipelines()
{
    ...
    tags_header.insert(ColumnWithTypeAndName{
        castToStringType(tags_metadata.columns.get(TimeSeriesColumnNames::MetricName).type), TimeSeriesColumnNames::MetricName});
    for (const auto & tag_name_and_column_name : tags_to_columns)
    {
        const auto & tuple = tag_name_and_column_name.safeGet<Tuple>();
    ...
    const auto & ts_array_type = assert_cast<const DataTypeArray &>(*getHeader().getByName(TimeSeriesColumnNames::TimeSeries).type);
    ...
    samples_header.insert(ColumnWithTypeAndName{ts_tuple_type.getElement(0), TimeSeriesColumnNames::Timestamp});
    samples_header.insert(ColumnWithTypeAndName{ts_tuple_type.getElement(1), TimeSeriesColumnNames::Value});
...
void TimeSeriesSink::consumeTagsAndSamples(const Block & block)
{
    ...
    const IColumn * input_metric_name_col = block.has(TimeSeriesColumnNames::MetricName)
        ? block.getByName(TimeSeriesColumnNames::MetricName).column.get() : nullptr;
    ...
    /// Step 2. Prepare columns for the Tags block.
    ...
    auto other_tags_names = tags_map_type->getKeyType()->createColumn();
    other_tags_names->reserve(num_rows);
    auto other_tags_values = tags_map_type->getValueType()->createColumn();
    other_tags_values->reserve(num_rows);
    auto other_tags_offsets = ColumnVector<IColumn::Offset>::create();
    other_tags_offsets->reserve(num_rows);
    ...
    /// Step 3. Prepare columns for the Samples block.
    ...
    for (size_t row = 0; row < num_rows; ++row)
    {
        ...
        sortTagsAndRemoveDuplicates(input_tags_keys, input_tags_values_col, input_tags_offsets, input_metric_name_col, row, sorted_tags);
        ...
        for (const auto & [tag_name, tag_value] : sorted_tags)
        {
            ...
            else
            {
                ...
                {
                    other_tags_names->insertData(tag_name.data(), tag_name.size());
                    other_tags_values->insertData(tag_value.data(), tag_value.size());
                }
            }
        }
    ...
    auto other_tags_column = ColumnMap::create(
        ColumnArray::create(ColumnTuple::create(std::move(tags_tuple_cols)), std::move(other_tags_offsets)));
    tags_block.insert(ColumnWithTypeAndName{std::move(other_tags_column), tags_map_type, TimeSeriesColumnNames::Tags});
    ...
    /// Step 7. Push to persistent pipelines.
...
void TimeSeriesSink::consumeMetrics(const Block & block)
{
    ...
    /// Step 2. Build the metrics block, skipping rows with empty metric_family.
    const auto & metrics_metadata = *time_series_storage.getTargetTable(ViewTarget::Metrics, getContext())->getInMemoryMetadataPtr();
    ...
    /// Step 3. Push to the persistent pipeline.
    Block metrics_block;
