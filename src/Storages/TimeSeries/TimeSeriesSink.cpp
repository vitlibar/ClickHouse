...
namespace
{
        ...
        IColumn & out_max_time_column)
    {
        for (size_t i = 0; i < filter.size(); ++i)
        {
            ...
            ts_timestamps.get(ts_start, min_time);
            max_time = min_time;
    ...
    size_t buildNonEmptyTagsFilter(const IColumn & metric_name_column, const ColumnArray::Offsets & tags_offsets, PaddedPODArray<UInt8> & filter)
    {
    ...
    /// Returns true if the column `metric_family` has at least one non-empty value.
    bool hasNonEmptyMetricFamily(const IColumn & column)
    {
...
void TimeSeriesSink::TargetPipeline::push(Block block)
{
    defaults_actions->execute(block);
    ...
    ViewTarget::Kind kind, const Block & header)
{
    ...
    /// Precompute actions to fill default values for any extra columns in the target table.
    const Block & target_header = pipeline->executor->getHeader();
    const auto & target_metadata = *time_series_storage.getTargetTable(kind, getContext())->getInMemoryMetadataPtr(getContext(), false);
    auto defaults_dag = addMissingDefaults(
        header,
        target_header.getNamesAndTypesList(),
        target_metadata.columns,
        pipeline->context);
    pipeline->defaults_actions = std::make_shared<ExpressionActions>(std::move(defaults_dag));
...
void TimeSeriesSink::consumeTagsAndSamples(const Block & block)
{
    ...
    chassert(new_metric_name_column->size() > 0);
    ...
    if (size_t total_samples = getTotalSamples(ts_offsets))
    {
...
void TimeSeriesSink::consumeMetrics(const Block & block)
{
    ...
    if (!hasNonEmptyMetricFamily(*metric_family_col.column))
        return; /// Nothing to insert if all metric_family values are empty.
    ...
    /// hasNonEmptyMetricFamily() checked that at least one non-empty `metric_family` presents.
    chassert(new_metric_family_column->size() > 0);
