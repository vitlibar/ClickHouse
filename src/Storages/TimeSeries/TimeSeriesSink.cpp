...
namespace
{
    ...
    /// Builds a block for the "tags" table from the series of the input block which pass `filter` (see buildNonEmptyTagsFilter):
    /// the columns `metric_name`, the columns of the tags from `tags_to_columns`, `tags`, and `min_time`, `max_time` if they are stored.
    /// The block has no `id` column yet, it is calculated from these columns by the caller.
    /// `tags_header` contains the types of all these columns.
