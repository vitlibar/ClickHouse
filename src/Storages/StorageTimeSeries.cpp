...
Tables created by older versions of ClickHouse (versions 0 and 1 of the table engine, see [Schema versioning](#schema-versioning))
store one sample per row in the columns `timestamp` and `value`; such tables can be read but not written into.
...
ENGINE = TimeSeries
SETTINGS version = 2, recent_samples_ttl_seconds = 345600, samples_bucket_step_seconds = 3600, recent_samples_bucket_step_seconds = 900
...
SAMPLES INNER ENGINE = AggregatingMergeTree PARTITION BY toYYYYMM(bucket) ORDER BY (id, bucket) SETTINGS index_granularity = 1024, index_granularity_bytes = 1048576, allow_dimensions_outside_sorting_key = 1
...
RECENT SAMPLES INNER ENGINE = AggregatingMergeTree PARTITION BY toStartOfInterval(bucket, toIntervalHour(5)) ORDER BY (id, bucket) TTL bucket + toIntervalSecond(346500) SETTINGS index_granularity = 1024, index_granularity_bytes = 1048576, allow_dimensions_outside_sorting_key = 1, ttl_only_drop_parts = 1
TAGS INNER COLUMNS
(
...
ORDER BY (id, bucket)
SETTINGS index_granularity = 1024, index_granularity_bytes = 1048576, allow_dimensions_outside_sorting_key = 1
```
...
SETTINGS index_granularity = 1024, index_granularity_bytes = 1048576, allow_dimensions_outside_sorting_key = 1, ttl_only_drop_parts = 1
```
...
| `samples_index_granularity` | UInt64 | 1024 | Sets `index_granularity` of the inner [samples](#samples-table) table. When set explicitly, it overrides `index_granularity` from the engine declaration. Ignored for an external samples table and a non-MergeTree engine |
| `samples_index_granularity_bytes` | UInt64 | 1048576 | Sets `index_granularity_bytes` of the inner [samples](#samples-table) table. When set explicitly, it overrides `index_granularity_bytes` from the engine declaration. Ignored for an external samples table and a non-MergeTree engine |
...
| `recent_samples_index_granularity` | UInt64 | 1024 | Sets `index_granularity` of the inner `recent samples` table. When set explicitly, it overrides `index_granularity` from the engine declaration. Ignored for an external recent samples table and a non-MergeTree engine. Requires `recent_samples_ttl_seconds` to be non-zero |
| `recent_samples_index_granularity_bytes` | UInt64 | 1048576 | Sets `index_granularity_bytes` of the inner `recent samples` table. When set explicitly, it overrides `index_granularity_bytes` from the engine declaration. Ignored for an external recent samples table and a non-MergeTree engine. Requires `recent_samples_ttl_seconds` to be non-zero |
...
The versions are:
- 0 - tables created before the `version` setting was introduced;
- 1 - the `version` setting was introduced;
- 2 - the [samples](#samples-table) and [recent samples](#recent-samples-table) tables store buckets of samples
  (columns `samples`, `bucket`, `min_time`, `max_time`) instead of one sample per row (columns `timestamp`, `value`).
  Tables of versions 0 and 1 can be read but not written into, and PromQL can't be evaluated over them.
