...
SAMPLES INNER ENGINE = AggregatingMergeTree PARTITION BY toYYYYMM(bucket) ORDER BY (id, bucket) SETTINGS index_granularity = 32768, allow_dimensions_outside_sorting_key = 1
...
RECENT SAMPLES INNER ENGINE = AggregatingMergeTree PARTITION BY toStartOfInterval(bucket, toIntervalHour(5)) ORDER BY (id, bucket) TTL bucket + toIntervalSecond(346500) SETTINGS index_granularity = 8192, allow_dimensions_outside_sorting_key = 1, ttl_only_drop_parts = 1
TAGS INNER COLUMNS
(
...
ORDER BY (id, bucket)
SETTINGS index_granularity = 32768, allow_dimensions_outside_sorting_key = 1
```
...
SETTINGS index_granularity = 8192, allow_dimensions_outside_sorting_key = 1, ttl_only_drop_parts = 1
```
