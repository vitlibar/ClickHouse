...
NamesAndTypesList StorageSystemEnabledRoles::getNamesAndTypes()
{
    NamesAndTypesList names_and_types{
        {"role_name", std::make_shared<DataTypeString>()},
        {"admin_option", std::make_shared<DataTypeUInt8>()},
        {"is_current", std::make_shared<DataTypeUInt8>()},
...
void StorageSystemEnabledRoles::fillData(MutableColumns & res_columns, const Context & context, const SelectQueryInfo &) const
{
    ...
    for (const auto & role_id : roles_info->enabled_roles)
    {
        ...
        bool admin_option = roles_info->enabled_roles_with_admin_option.contains(role_id);
        bool is_current = roles_info->current_roles.contains(role_id);
