...
void MergeTreeData::restoreDataFromBackup(RestorerFromBackup & restorer, const String & data_path_in_backup, const std::optional<ASTs> & partitions)
{
    ...
    if (!restorer.getRestoreSettings().with_mutations)
...
String MergeTreeData::getPartitionIDFromQuery(const ASTPtr & ast, ContextPtr local_context, DataPartsLock * acquired_lock) const
{
    ...
    auto [partition_id, partition_value] = getPartitionIDAndValueFromQuery(ast, local_context, *metadata, format_version);

    if (partition_value)
    {
        auto data_parts_lock = (acquired_lock) ? DataPartsLock() : lockParts();
        DataPartPtr existing_part_in_partition = getAnyPartInPartition(partition_id, data_parts_lock);
        if (existing_part_in_partition && existing_part_in_partition->partition.value != partition_value->value)
        {
            WriteBufferFromOwnString buf;
            partition_value->serializeText(*this, buf, FormatSettings{});
...
std::pair<String, std::optional<MergeTreePartition>> MergeTreeData::getPartitionIDAndValueFromQuery(
    const ASTPtr & ast,
    const ContextPtr & local_context,
    ...
    MergeTreeDataFormatVersion format_version_)
{
    ...
    {
        MergeTreePartInfo::validatePartitionID(partition_ast.id, format_version_);
        return {partition_ast.id, std::nullopt};
    }

    if (format_version_ < MERGE_TREE_DATA_MIN_FORMAT_VERSION_WITH_CUSTOM_PARTITIONING)
    {
        ...
        if (partition_lit && partition_lit->value.getType() == Field::Types::String)
        {
            ...
            MergeTreePartInfo::validatePartitionID(partition_id, format_version_);
            return {partition_id, std::nullopt};
        }
    }

    /// Re-parse partition key fields using the information about expected field types.
    const Block & key_sample_block = metadata.getPartitionKey().sample_block;
    ...
    std::optional<MergeTreePartition> partition_value(std::move(partition_row));
    String partition_id = partition_value->getID(key_sample_block);
    return {std::move(partition_id), std::move(partition_value)};
}
...
std::unordered_set<String> MergeTreeData::getPartitionIdsAffectedByCommands(
    const MutationCommands & commands,
    const ContextPtr & query_context,
    const StorageInMemoryMetadata & metadata,
    MergeTreeDataFormatVersion format_version_)
{
    std::unordered_set<String> affected_partition_ids;
    ...
    for (const auto & command : commands)
    {
        ...
        affected_partition_ids.insert(
            getPartitionIDAndValueFromQuery(command.partition, query_context, metadata, format_version_).first
        );
    }
