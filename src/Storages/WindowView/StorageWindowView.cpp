    ...
    , clean_interval_usec(context_->getSettingsRef().window_view_clean_interval.totalMicroseconds())
{
    ...
    const auto & target_info = query.getTarget(TargetTableKind::kTarget);
    const auto & to_table_id = target_info.table_id;
    has_inner_target_table = to_table_id.empty();

    if (has_inner_target_table && !target_info.storage)
    ...
    const auto & inner_info = query.getTarget(TargetTableKind::kIntermediate);
    if (inner_info.storage)
        inner_table_engine = inner_info.storage->clone();
    ...
    if (mode < LoadingStrictnessLevel::ATTACH)
    {
        ...
        if (has_inner_target_table)
        {
            ...
            target_create_query->set(target_create_query->storage, target_info.storage->ptr());
