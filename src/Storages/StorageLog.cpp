...
private:
    ...
    struct Stream
    {
        ...
        /// Set to not write shared offsets of columns for nested structures multiple times
        bool written = false;
...
void LogSink::writeData(const NameAndTypePair & name_and_type, const IColumn & column)
{
    ...
    serialization->enumerateStreams([&] (const ISerialization::SubstreamPath & path)
    {
        ...
        auto & stream = streams.try_emplace(
            stream_name,
            storage.disk,
            storage.files_by_stream_name[stream_name].data_file_path,
            columns.getCodecOrDefault(name_and_type.name),
            storage.max_compress_block_size).first->second;
    ...
    if (storage.use_marks)
    {
        serialization->enumerateStreams([&] (const ISerialization::SubstreamPath & path)
        {
            ...
            auto & marks = storage.files_by_stream_name[stream_name].marks;
...
void StorageLog::loadMarks(const WriteLock & /* already locked for writing */)
{
    ...
    if (!use_marks)
    {
        /// Use generated marks.
        Mark mark;
        mark.rows = std::numeric_limits<UInt64>::max();
        mark.offset = 0;
        for (auto * file : files)
            file->marks.push_back(mark);
        marks_loaded = true;
        return;
    }
