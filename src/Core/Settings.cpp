...
#define COMMON_SETTINGS(M, ALIAS) \
    ...
    M(UInt64, backup_restore_keeper_max_retries_while_handling_error, 20, "Max retries for [Zoo]Keeper operations during while handling an error of a BACKUP ON CLUSTER or RESTORE ON CLUSTER operation", 0) \
    M(UInt64, backup_restore_finish_timeout_after_error_sec, 180, "How long the initiator should wait for other host to handle the 'error' node and finish their work", 0) \
