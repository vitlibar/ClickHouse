...
void Settings::dumpToArrayColumns(IColumn * column_names_, IColumn * column_values_, bool with_unchanged)
{
    ...
    for (auto it = begin(with_unchanged); it != end(); ++it)
    {
        if (column_names)
        {
        ...
        if (column_values)
            column_values->getData().insert(it.getValueAsString());
        ++count;
    }
...
{
    for (auto field : allIncludingUnchanged())
    {
