...
void SettingFieldNumber<T>::readBinary(ReadBuffer & in)
{
    ...
    else
    {
        ...
        *this = ::DB::parseFromString<T>(value);
    }
}
...
{
    return *this = fieldToMaxThreads(f);
}
...
{
    return *this = fieldToNumber<UInt64>(f);
}
