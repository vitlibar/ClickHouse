...
/// Function timeSeriesGrid([value1, value2, value3, ...], start_timestamp, step) converts array of values [value1, value2, value3, ...]
...
public:
    ...
    static void checkDataTypes(const ColumnsWithTypeAndName & arguments)
    {
        ...
        const auto & values_array_type = arguments[0].type;
        const auto & start_timestamp_type = arguments[1].type;
        ...
        if (!isArray(values_array_type))
            throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Argument #{} of function {} has wrong type {}, it must be {}",
                            1, name, values_array_type, "an array of values");
        ...
        if (!(isDateTimeOrDateTime64(start_timestamp_type) || isUInt32(start_timestamp_type)))
            throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Argument #{} (start_timestamp) of function {} has wrong type {}, it must be {}",
                            2, name, start_timestamp_type, "DateTime64 or DateTime or UInt32");
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr & result_type, size_t input_rows_count) const override
    {
        ...
        const auto & column_values_array = *arguments[0].column;
        const auto & values_array_type = arguments[0].type;
        const auto & column_start_timestamp = *arguments[1].column;
        const auto & start_timestamp_type = arguments[1].type;
        const auto & column_step = *arguments[2].column;
        const auto & step_type = arguments[2].type;
        ...
        if (isDateTime64(start_timestamp_type))
            return doExecute<DateTime64, Decimal64>(column_values_array, values_are_nullable, column_start_timestamp, column_step, step_extra_scale, result_type, input_rows_count);
        else if (isDateTime(start_timestamp_type) || isUInt32(start_timestamp_type))
            return doExecute<UInt32, Int32>(column_values_array, values_are_nullable, column_start_timestamp, column_step, step_extra_scale, result_type, input_rows_count);
        else
            throw Exception(ErrorCodes::ILLEGAL_COLUMN,
                            "Illegal column {} of argument #{} of function {}, it must be {}",
                            column_start_timestamp.getName(), 2, name, "DateTime64 or DateTime or UInt32");
    }

    template <typename TimestampType, typename IntervalType>
    static ColumnPtr doExecute(const IColumn & column_values_array, bool values_are_nullable,
                               const IColumn & column_start_timestamp,
                               ...
                               size_t num_rows)
    {
        ...
        if (!array_column || (values_are_nullable && !checkColumn<ColumnNullable>(&array_column->getData())))
        {
            ...
            if (full_column.get() != &column_values_array)
                return doExecute<TimestampType, IntervalType>(*full_column, values_are_nullable, column_start_timestamp, column_step, step_extra_scale, result_type, num_rows);
            throw Exception(ErrorCodes::ILLEGAL_COLUMN,
                            "Illegal column {} of argument #{} of function {}, it must be {}",
...
REGISTER_FUNCTION(TimeSeriesGrid)
{
    ...
    FunctionDocumentation::Syntax syntax = "timeSeriesGrid(values, start_timestamp, step)";
    FunctionDocumentation::Arguments arguments = {{"values", "array of values", {"Array(Nullable(Float64))", "Array(Float64)", "Array(Nullable(Float32))", "Array(Float32)"}},
    ...
    FunctionDocumentation::Examples examples = {{"Example", "SELECT timeSeriesGrid([10, 20, NULL, 30], '2025-06-01 00:00:00'::DateTime64(3), 30)", "[('2025-06-01 00:00:00.000',10),('2025-06-01 00:00:30.000',20),('2025-06-01 00:01:30.000',30)]"}};
