...
public:
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr &, size_t input_rows_count) const override
    {
        ...
        /// Compute the total number of result inner arrays across rows for exact reservation.
        size_t total_result_inner_arrays = 0;
        for (size_t r = 0; r != input_rows_count; ++r)
        {
            ColumnArray::Offset outer_start = outer_offsets[r - 1];
            ColumnArray::Offset outer_end = outer_offsets[r];
            if (outer_start < outer_end)
                total_result_inner_arrays += inner_offsets[outer_start] - inner_offsets[outer_start - 1];
        }
        ...
        for (size_t r = 0; r != input_rows_count; ++r)
        {
            ...
            size_t m = outer_end - outer_start;

            /// Size of each inner array (number of columns in the input matrix)
            size_t n = 0;

            if (m > 0)
            {
                n = inner_offsets[outer_start] - inner_offsets[outer_start - 1];

                for (ColumnArray::Offset k = outer_start + 1; k < outer_end; ++k)
                {
                    size_t nk = inner_offsets[k] - inner_offsets[k - 1];
                    if (nk != n)
                        throw Exception(
