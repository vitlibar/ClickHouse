...
StoragePtr TableFunctionTimeSeriesTarget<target_kind>::getTargetTable(const ContextPtr & context) const
{
    ...
    auto time_series_storage = typeid_cast<std::shared_ptr<StorageTimeSeries>>(storage);
    if (!time_series_storage)
    {
        throw Exception(
            ErrorCodes::WRONG_TABLE_ENGINE,
            "Table function {} can be used with a TimeSeries table engine only, {} has engine {}",
            name,
...
{
    factory.registerFunction<TableFunctionTimeSeriesTarget<TargetTableKind::kData>>();
    factory.registerFunction<TableFunctionTimeSeriesTarget<TargetTableKind::kTags>>();
    factory.registerFunction<TableFunctionTimeSeriesTarget<TargetTableKind::kMetrics>>();
}
