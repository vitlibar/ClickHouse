...
int mainEntryExampleTimeSeriesToGridTwoStackVsRecompute(int, char **)
{
    ...
    /// Maximum (`timeSeriesMaxToGrid` / `timeSeriesTimestampOfMaxToGrid` differ only in the tie-break; the buckets are summaries).
    ...
    /// Minimum (`timeSeriesMinToGrid` / `timeSeriesTimestampOfMinToGrid` differ only in the tie-break; the buckets are raw samples).
