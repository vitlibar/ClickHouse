...
Dataset<Bucket> buildDataset()
{
    ...
    for (size_t series = 0; series < NUM_SERIES; ++series)
    {
        ...
        {
            Bucket bucket;
            bucket.add(DateTime64(static_cast<Int64>(k) * STEP), static_cast<Float64>((k * 7 + series * 3) % 101));
    ...
    for (int r = 0; r < REPEATS; ++r)
    {
        ...
        for (const auto & buckets : dataset)
        {
            ...
            for (size_t i = 0; i < BASE_GRID; ++i)
            {
                ...
                if (it != buckets.end())
                    aggregator.add(it->second, DateTime64(static_cast<Int64>(i) * STEP));
                if (i >= buckets_per_window)
                    aggregator.removeBefore(DateTime64((static_cast<Int64>(i) - static_cast<Int64>(buckets_per_window)) * STEP));
                const auto result = aggregator.getResult(DateTime64(static_cast<Int64>(i) * STEP));
                if (result)
