...
public:
    ...
    using ElementType = std::pair<TimestampType, ValueType>;
    ...
    struct Data
    {
        ...
        void reserve(size_t new_size, Arena * arena)
        {
            if (new_size > allocated_size)
            {
                ...
                elements = arena->realloc(elements, old_size * sizeof(ElementType), allocated_size * sizeof(ElementType));
            }
        }
        ...
        {
            reserve(size + 1);
            element[size++] = std::make_pair(timestamp, value);
        }

        void merge(const Data & rhs, Arena * arena)
        {
            reserve(size + rhs.size);
            memcpy(elements + size, rhs.elements, rhs.size * sizeof(ElementType));
            size += rhs.size;
        }
        ...
        void sortAndRemoveDuplicates()
        {
            ...
            auto less_by_timestamp = [](const ElementType & left, const ElementType & right) {
            ...
            {
                if (elements[i].timestamp == element[i - 1].timestamp)
                {
            ...
            {
                auto equal_by_timestamp = [](const ElementType & left, const ElementType & right) {
                    return left.timestamp == right.timestamp;
                };

                const ElementType * new_end = std::unique(elements, elements + size, equal_by_timestamp);
    ...
    void NO_SANITIZE_UNDEFINED ALWAYS_INLINE reserveAdd(AggregateDataPtr __restrict place, size_t num_elements_to_add) const
    {
        Data & data = this->data(place);
        data.reserve(data.size + num_elements_to_add);
    }
    ...
    void serialize(ConstAggregateDataPtr __restrict place, WriteBuffer & buf, std::optional<size_t> /* version */) const override
    {
        ...
        for (size_t i = 0; i < data.size; ++i)
            writeBinaryLittleEndian(data.elements[i], buf);
    }
    ...
    void deserialize(AggregateDataPtr __restrict place, ReadBuffer & buf, std::optional<size_t> /* version */, Arena * arena) const override
    {
        ...
        for (size_t i = 0; i < size; ++i)
            readBinaryLittleEndian(data.elements[i], buf);
