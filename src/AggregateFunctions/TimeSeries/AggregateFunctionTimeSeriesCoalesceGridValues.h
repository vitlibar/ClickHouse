...
public:
    ...
    explicit AggregateFunctionTimeSeriesCoalesceGridValues(
        std::shared_ptr<const ContextTimeSeriesTagsCollector> tags_collector_, const DataTypes & argument_types_, Mode mode_)
        : Base(argument_types_, {}, createResultType(argument_types_))
        , tags_collector(tags_collector_)
        , mode(mode_)
    ...
    [[noreturn]] void throwFoundDuplicateSeries(Group first_group, Group second_group) const
    {
        ...
        if (has_group_argument)
        {
            ...
            {
                auto tags = tags_collector->getTagsByGroup(first_group);
                extra_info = fmt::format(" with tags {}", ContextTimeSeriesTagsCollector::toString(tags));
            }
            else
            {
                auto first_tags = tags_collector->getTagsByGroup(first_group);
                auto second_tags = tags_collector->getTagsByGroup(second_group);
                extra_info = fmt::format(" with tags {} and {}", ContextTimeSeriesTagsCollector::toString(first_tags), ContextTimeSeriesTagsCollector::toString(second_tags));
            }
        }
