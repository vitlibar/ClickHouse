#include <AggregateFunctions/TimeSeries/AggregateFunctionTimeSeriesGroupArraySorted.h>
...
AggregateFunctionPtr createWithValueType(const std::string & name, const DataTypes & argument_types)
{
    ...
    {
        res = std::make_shared<AggregateFunctionTimeSeriesGroupArraySorted<DateTime64, ValueType, array_arguments>>(argument_types);
    }
    else if (isDateTime(timestamp_type) || isUInt32(timestamp_type))
    {
        res = std::make_shared<AggregateFunctionTimeSeriesGroupArraySorted<UInt32, ValueType, array_arguments>>(argument_types);
    }
...
AggregateFunctionPtr createAggregateFunctionTimeseriesGroupArraySorted(const std::string & name, const DataTypes & argument_types, const Array & parameters, const Settings * settings)
{
    ...
    {
        if (array_arguments)
            res = createWithValueType<true, Float64>(name, argument_types, parameters);
        else
            res = createWithValueType<false, Float64>(name, argument_types, parameters);
    }
    else if (value_type->getTypeId() == TypeIndex::Float32)
    {
        if (array_arguments)
            res = createWithValueType<true, Float32>(name, argument_types, parameters);
        else
            res = createWithValueType<false, Float32>(name, argument_types, parameters);
    }
    else
    {
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
...
{
    factory.registerFunction("timeSeriesGroupArraySorted", createAggregateFunctionTimeseriesGroupArraySorted);
}
