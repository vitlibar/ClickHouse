...
/// Aggregate function taking arrays of nullable values of the same size, and then returning a new array of the same size,
/// consisted of elements of the source arrays which are not NULL.
/// If different source array contain non-NULL values at the same position the function throws an exception,
/// or if `throw_if_conflict == false` just quietly sets NaN in the result vector.

template <typename ValueType, bool or_null>
class AggregateFunctionTimeSeriesGroupValuesFromGrid final :
    public IAggregateFunctionHelper<AggregateFunctionTimeSeriesGroupValuesFromGrid<ValueType, or_null>>
{
public:
    using Base = IAggregateFunctionHelper<AggregateFunctionTimeSeriesGroupValuesFromGrid<ValueType, or_null>>;

    using ColVecType = ColumnVectorOrDecimal<ValueType>;

    static const constexpr std::string_view name = or_null ? "timeSeriesGroupValuesFromGridOrNull" : "timeSeriesGroupValuesFromGrid";
    ...
    explicit AggregateFunctionTimeSeriesGroupValuesFromGrid(const DataTypes & argument_types_)
    ...
    void insertResultInto(AggregateDataPtr __restrict place, IColumn & to, Arena *) const override
    {
        ...
        for (size_t i = 0; i != data.size; ++i)
        {
            ...
            else if ((element.num_values == 0) || or_null)
            {
