...
def test_dcl_introspection():
    ...
    assert instance.query("SHOW CREATE QUOTA myQuota") == "CREATE QUOTA myQuota KEYED BY \\'user name\\' SET MAX QUERIES = 1000, MAX READ ROWS = 1000 FOR INTERVAL 1 YEAR TO default\n"
    ...
    copy_quota_xml('two_intervals.xml')
    assert instance.query("SHOW QUOTAS") == "myQuota\n"
    assert instance.query("SHOW CREATE QUOTA myQuota") == "CREATE QUOTA myQuota KEYED BY \\'user name\\' SET MAX QUERIES = 1000, MAX READ ROWS = 1000 FOR INTERVAL 1 YEAR, SET MAX RESULT BYTES = 30000, MAX READ BYTES = 20000, MAX EXECUTION TIME = 120 FOR RANDOMIZED INTERVAL 2 YEAR TO default\n"
    expected_usage = "myQuota key=\\\\'default\\\\' interval=\[.*\] queries=1/1000 errors=0 result_rows=50 result_bytes=200 read_rows=50/1000 read_bytes=200 execution_time=.*\n"\
...
def test_dcl_management():
    ...
    instance.query("CREATE QUOTA qA SET MAX QUERIES = 123 FOR INTERVAL 15 MONTH TO CURRENT_USER")
    assert instance.query("SHOW QUOTAS") == "qA\n"
    assert instance.query("SHOW CREATE QUOTA qA") == "CREATE QUOTA qA KEYED BY \\'none\\' SET MAX QUERIES = 123 FOR INTERVAL 5 QUARTER TO default\n"
    ...
    instance.query("ALTER QUOTA qA SET MAX QUERIES = 321, MAX ERRORS = 10 FOR INTERVAL 15 MONTH, SET MAX EXECUTION TIME = 0.5 FOR INTERVAL 0.5 HOUR")
    assert instance.query("SHOW CREATE QUOTA qA") == "CREATE QUOTA qA KEYED BY \\'none\\' SET MAX EXECUTION TIME = 0.5 FOR INTERVAL 30 MINUTE, SET MAX QUERIES = 321, MAX ERRORS = 10 FOR INTERVAL 5 QUARTER TO default\n"
    ...
    instance.query("ALTER QUOTA qA UNSET TRACKING FOR INTERVAL 15 MONTH, SET TRACKING FOR RANDOMIZED INTERVAL 16 MONTH, UNSET TRACKING FOR INTERVAL 1800 SECOND")
    assert instance.query("SHOW CREATE QUOTA qA") == "CREATE QUOTA qA KEYED BY \\'none\\' SET TRACKING FOR RANDOMIZED INTERVAL 16 MONTH TO default\n"
    ...
    assert instance.query("SHOW CREATE QUOTA qB") == "CREATE QUOTA qB KEYED BY \\'none\\' SET TRACKING FOR RANDOMIZED INTERVAL 16 MONTH TO default\n"
