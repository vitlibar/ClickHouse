...
namespace
{
    ...
    bool parseDefaultRoles(IParserBase::Pos & pos, Expected & expected, bool id_mode, std::shared_ptr<ASTGeneralizedRoleSet> & default_roles)
    {
        return IParserBase::wrapParseImpl(pos, [&]
        {
            ...
            ASTPtr ast;
            if (!ParserGeneralizedRoleSet{}.enableCurrentUserKeyword(false).enableIDMode(id_mode).parse(pos, ast, expected))
                return false;

            default_roles = typeid_cast<std::shared_ptr<ASTGeneralizedRoleSet>>(ast);
            return true;
        });
    }
