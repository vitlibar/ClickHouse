...
namespace
{
    bool parseRoles(IParserBase::Pos & pos, Expected & expected, std::shared_ptr<ASTGeneralizedRoleSet> & roles)
    {
        return IParserBase::wrapParseImpl(pos, [&]
        {
            ASTPtr ast;
            if (!ParserGeneralizedRoleSet{}.enableCurrentUserKeyword(false).parse(pos, ast, expected))
                return false;
    ...
    bool parseToUsers(IParserBase::Pos & pos, Expected & expected, std::shared_ptr<ASTGeneralizedRoleSet> & to_users)
    {
        return IParserBase::wrapParseImpl(pos, [&]
        {
            ...
            ASTPtr ast;
            if (!ParserGeneralizedRoleSet{}.enableAllKeyword(false).parse(pos, ast, expected))
                return false;

            to_users = typeid_cast<std::shared_ptr<ASTGeneralizedRoleSet>>(ast);
            return true;
        });
    }
}
...
bool ParserSetRoleQuery::parseImpl(Pos & pos, ASTPtr & node, Expected & expected)
{
    ...
    std::shared_ptr<ASTGeneralizedRoleSet> to_users;
