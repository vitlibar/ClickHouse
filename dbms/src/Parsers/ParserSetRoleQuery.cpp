...
namespace
{
    bool parseRoles(IParserBase::Pos & pos, Expected & expected, std::shared_ptr<ASTRoleList> & roles)
    {
        ...
        {
            ASTPtr ast;
            if (!ParserRoleList{/* allow_current_user = */ false, /* allow_all = */ true}.parse(pos, ast, expected))
                return false;
    ...
    bool parseToUsers(IParserBase::Pos & pos, Expected & expected, std::shared_ptr<ASTRoleList> & to_users)
    {
        return IParserBase::wrapParseImpl(pos, [&]
        {
            ...
            ASTPtr ast;
            if (!ParserRoleList{/* allow_current_user = */ true, /* allow_all = */ true}.parse(pos, ast, expected))
                return false;

            to_users = typeid_cast<std::shared_ptr<ASTRoleList>>(ast);
            return true;
        });
    }
}
...
bool ParserSetRoleQuery::parseImpl(Pos & pos, ASTPtr & node, Expected & expected)
{
    ...
    std::shared_ptr<ASTRoleList> to_users;
