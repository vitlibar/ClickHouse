...
namespace
{
    ...
    bool parseToRoles(IParserBase::Pos & pos, Expected & expected, bool id_mode, std::shared_ptr<ASTGeneralizedRoleSet> & roles)
    {
        return IParserBase::wrapParseImpl(pos, [&]
        {
            ASTPtr node;
            if (roles || !ParserKeyword{"TO"}.ignore(pos, expected) || !ParserGeneralizedRoleSet{}.enableIDMode(id_mode).parse(pos, node, expected))
                return false;

            roles = std::static_pointer_cast<ASTGeneralizedRoleSet>(node);
            return true;
        });
    }
}
