    ...
    , intermediate_commit(intermediate_commit_)
{
    ...
    activate_on_add_view
        = global_context.getViewDependencies().subscribeForChanges([this](const DatabaseAndTableNameRef & from, const DatabaseAndTableNameRef &, const Strings & from_columns)
    {
        if (!from_columns.empty() && (from.first == database_name) && (from.second == table_name))
