...
private:
    ...
    struct Node
    {
        ...
        template <typename... Args>
        Node(std::string_view keyword_, size_t flag_, Level level_, NodePtr first_child, Args &&... other_children)
            : Node(keyword_, std::move(first_child), std::move(other_children)...)
        {
            flags.set(flag_);
            level = std::max(level, level_);
        }
    };
    ...
    static void makeFlagsToKeywordTree(NodePtr & flags_to_keyword_tree_)
    {
        ...
        ext::push_back(attach_partition->aliases, "ALTER ATTACH PARTITION", "ATTACH PART", "ALTER ATTACH PART");
        auto detach_partition = std::make_unique<Node>("DETACH PARTITION", next_flag++, TABLE_LEVEL);
        ext::push_back(detach_partition->aliases, "ALTER DETACH PARTITION");
        auto drop_partition = std::make_unique<Node>("DROP PARTITION", next_flag++, TABLE_LEVEL);
        ext::push_back(drop_partition->aliases, "ALTER DROP PARTITION", "DROP DETACHED PARTITION", "ALTER DROP DETACHED PARTITION", "DROP DETACHED PART", "ALTER DROP DETACHED PART");
        ...
        ext::push_back(move_partition->aliases, "ALTER MOVE PARTITION TO DISK", "MOVE PART TO DISK", "ALTER MOVE PART TO DISK", "MOVE PARTITION TO VOLUME", "ALTER MOVE PARTITION TO VOLUME", "MOVE PART TO VOLUME", "ALTER MOVE PART TO VOLUME");
        ...
        ext::push_back(partition->aliases, "ALTER PARTITION");

        auto alter_table = std::make_unique<Node>("ALTER TABLE", std::move(update), std::move(delet), std::move(alter_column), std::move(index), std::move(alter_constraint), std::move(modify_ttl), std::move(modify_setting), std::move(partition));

        auto refresh_live_view = std::make_unique<Node>("REFRESH LIVE VIEW", next_flag++, TABLE_LEVEL);
        ext::push_back(refresh_live_view->aliases, "ALTER LIVE VIEW REFRESH");
        ...
        auto create_table = std::make_unique<Node>("CREATE TABLE", next_flag++, TABLE_LEVEL, std::move(create_temporary_table));
