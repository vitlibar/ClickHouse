...
std::shared_ptr<const EnabledRowPolicies> RowPolicyCache::getEnabledRowPolicies(const UUID & user_id, const std::vector<UUID> & enabled_roles)
{
    ...
    auto res = std::shared_ptr<EnabledRowPolicies>(new EnabledRowPolicies(user_id, enabled_roles));
    enabled_row_policies.push_back(res);
...
void RowPolicyCache::mixConditions()
{
    ...
    boost::range::remove_erase_if(
        ...
        [&](const std::weak_ptr<EnabledRowPolicies> & weak)
        {
            auto elem = weak.lock();
            if (!elem)
                return true; // remove from the `enabled_row_policies` list.
            mixConditionsFor(*elem);
            return false; // keep in the `enabled_row_policies` list.
        });
}
