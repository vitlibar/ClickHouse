...
bool operator ==(const EnabledRoles::Params & lhs, const EnabledRoles::Params & rhs)
{
    return (lhs.current_roles == rhs.current_roles) && (lhs.current_roles_with_admin_option == rhs.current_roles_with_admin_option);
}

bool operator <(const EnabledRoles::Params & lhs, const EnabledRoles::Params & rhs)
{
    if (lhs.current_roles < rhs.current_roles)
        return true;
    if (lhs.current_roles > rhs.current_roles)
        return false;
    return (lhs.current_roles_with_admin_option < rhs.current_roles_with_admin_option);
}
