...
bool AccessRightsContext::checkImpl(Poco::Logger * log, const AccessFlags & access, const Args &... args) const
{
    std::lock_guard lock{mutex};
    const auto & result_access = calculateResultAccess();

    LOG_INFO(
        &Poco::Logger::get("AccessRightsContext (" + user_name + ")"),
        ("Checking access rights " + AccessRightsElement{access, args...}.toString()));
    ...
    auto show_error = [&](const String & msg, [[maybe_unused]] int error_code)
    {
        if constexpr (mode == THROW_IF_ACCESS_DENIED)
            ...
            LOG_WARNING(log, msg + formatSkippedMessage(args...));
    };
...
template <int mode>
bool AccessRightsContext::checkImpl(Poco::Logger * log, const AccessRightsElement & element) const
{
    if (element.any_database)
    {
        return checkImpl<mode>(log, element.access_flags);
    }
    else if (element.any_table)
    {
        if (element.current_database)
            return checkImpl<mode>(log, element.access_flags, current_database);
        else
            return checkImpl<mode>(log, element.access_flags, element.database);
    }
    else if (element.any_column)
    {
        if (element.current_database)
            return checkImpl<mode>(log, element.access_flags, current_database, element.table);
        else
            return checkImpl<mode>(log, element.access_flags, element.database, element.table);
    }
    else
    {
        if (element.current_database)
            return checkImpl<mode>(log, element.access_flags, current_database, element.table, element.columns);
        else
            return checkImpl<mode>(log, element.access_flags, element.database, element.table, element.columns);
    }
}


template <int mode>
bool AccessRightsContext::checkImpl(Poco::Logger * log, const AccessRightsElements & elements) const
{
    for (const auto & element : elements)
        if (!checkImpl<mode>(log, element))
            return false;
    return true;
}
...
void AccessRightsContext::check(const AccessFlags & access, CurrentDatabaseTag) const { checkImpl<THROW_IF_ACCESS_DENIED>(nullptr, access, current_database); }
void AccessRightsContext::check(const AccessFlags & access, CurrentDatabaseTag, const std::string_view & table) const { checkImpl<THROW_IF_ACCESS_DENIED>(nullptr, access, current_database, table); }
void AccessRightsContext::check(const AccessFlags & access, CurrentDatabaseTag, const std::string_view & table, const std::string_view & column) const { checkImpl<THROW_IF_ACCESS_DENIED>(nullptr, access, current_database, table, column); }
void AccessRightsContext::check(const AccessFlags & access, CurrentDatabaseTag, const std::string_view & table, const std::vector<std::string_view> & columns) const { checkImpl<THROW_IF_ACCESS_DENIED>(nullptr, access, current_database, table, columns); }
void AccessRightsContext::check(const AccessFlags & access, CurrentDatabaseTag, const std::string_view & table, const Strings & columns) const { checkImpl<THROW_IF_ACCESS_DENIED>(nullptr, access, current_database, table, columns); }
...
bool AccessRightsContext::isGranted(const AccessFlags & access, CurrentDatabaseTag) const { return checkImpl<RETURN_FALSE_IF_ACCESS_DENIED>(nullptr, access, current_database); }
bool AccessRightsContext::isGranted(const AccessFlags & access, CurrentDatabaseTag, const std::string_view & table) const { return checkImpl<RETURN_FALSE_IF_ACCESS_DENIED>(nullptr, access, current_database, table); }
bool AccessRightsContext::isGranted(const AccessFlags & access, CurrentDatabaseTag, const std::string_view & table, const std::string_view & column) const { return checkImpl<RETURN_FALSE_IF_ACCESS_DENIED>(nullptr, access, current_database, table, column); }
bool AccessRightsContext::isGranted(const AccessFlags & access, CurrentDatabaseTag, const std::string_view & table, const std::vector<std::string_view> & columns) const { return checkImpl<RETURN_FALSE_IF_ACCESS_DENIED>(nullptr, access, current_database, table, columns); }
bool AccessRightsContext::isGranted(const AccessFlags & access, CurrentDatabaseTag, const std::string_view & table, const Strings & columns) const { return checkImpl<RETURN_FALSE_IF_ACCESS_DENIED>(nullptr, access, current_database, table, columns); }
...
bool AccessRightsContext::isGranted(Poco::Logger * log, const AccessFlags & access) const { return checkImpl<LOG_WARNING_IF_ACCESS_DENIED>(log, access); }
bool AccessRightsContext::isGranted(Poco::Logger * log, const AccessFlags & access, const std::string_view & database) const { return checkImpl<LOG_WARNING_IF_ACCESS_DENIED>(log, access, database); }
bool AccessRightsContext::isGranted(Poco::Logger * log, const AccessFlags & access, const std::string_view & database, const std::string_view & table) const { return checkImpl<LOG_WARNING_IF_ACCESS_DENIED>(log, access, database, table); }
bool AccessRightsContext::isGranted(Poco::Logger * log, const AccessFlags & access, const std::string_view & database, const std::string_view & table, const std::string_view & column) const { return checkImpl<LOG_WARNING_IF_ACCESS_DENIED>(log, access, database, table, column); }
bool AccessRightsContext::isGranted(Poco::Logger * log, const AccessFlags & access, const std::string_view & database, const std::string_view & table, const std::vector<std::string_view> & columns) const { return checkImpl<LOG_WARNING_IF_ACCESS_DENIED>(log, access, database, table, columns); }
bool AccessRightsContext::isGranted(Poco::Logger * log, const AccessFlags & access, const std::string_view & database, const std::string_view & table, const Strings & columns) const { return checkImpl<LOG_WARNING_IF_ACCESS_DENIED>(log, access, database, table, columns); }
bool AccessRightsContext::isGranted(Poco::Logger * log, const AccessFlags & access, CurrentDatabaseTag) const { return checkImpl<LOG_WARNING_IF_ACCESS_DENIED>(log, access, current_database); }
bool AccessRightsContext::isGranted(Poco::Logger * log, const AccessFlags & access, CurrentDatabaseTag, const std::string_view & table) const { return checkImpl<LOG_WARNING_IF_ACCESS_DENIED>(log, access, current_database, table); }
bool AccessRightsContext::isGranted(Poco::Logger * log, const AccessFlags & access, CurrentDatabaseTag, const std::string_view & table, const std::string_view & column) const { return checkImpl<LOG_WARNING_IF_ACCESS_DENIED>(log, access, current_database, table, column); }
bool AccessRightsContext::isGranted(Poco::Logger * log, const AccessFlags & access, CurrentDatabaseTag, const std::string_view & table, const std::vector<std::string_view> & columns) const { return checkImpl<LOG_WARNING_IF_ACCESS_DENIED>(log, access, current_database, table, columns); }
bool AccessRightsContext::isGranted(Poco::Logger * log, const AccessFlags & access, CurrentDatabaseTag, const std::string_view & table, const Strings & columns) const { return checkImpl<LOG_WARNING_IF_ACCESS_DENIED>(log, access, current_database, table, columns); }
bool AccessRightsContext::isGranted(Poco::Logger * log, const AccessRightsElement & access) const { return checkImpl<LOG_WARNING_IF_ACCESS_DENIED>(log, access); }
bool AccessRightsContext::isGranted(Poco::Logger * log, const AccessRightsElements & access) const { return checkImpl<LOG_WARNING_IF_ACCESS_DENIED>(log, access); }
...
const AccessRights & AccessRightsContext::calculateResultAccess(UInt64 readonly_, bool allow_ddl_, bool allow_introspection_) const
{
    ...
    static const AccessFlags read_table_access = AccessType::SELECT;
    ...
    /// Apply access to temporary tables.
    if (result.isGranted(AccessType::CREATE_TEMPORARY_TABLE))
        result.grant(table_ddl | read_table_access | write_table_access, "" /* temporary tables use empty database */);
    else
        result.fullRevoke(table_ddl | read_table_access | write_table_access, "");
