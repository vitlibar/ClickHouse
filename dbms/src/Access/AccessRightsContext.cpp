...
AccessRightsContext::AccessRightsContext()
{
    ...
    quota = std::make_shared<QuotaContext>();
    row_policy = std::make_shared<RowPolicyContext>();
}
    ...
    , params(params_)
{
    ...
    setUser(manager->read<User>(getUserID()));
}
...
void AccessRightsContext::setUser(const UserPtr & user_) const
{
    ...
    if (!user)
    {
        ...
        user = user_;
        trace_log = &Poco::Logger::get("AccessRightsContext (" + getUserName() + ")");
        row_policy = manager->getRowPolicyContext(getUserID());
        quota = manager->getQuotaContext(getUserID(), getUserName(), params.address, params.quota_key);
        return;
    }
    ...
    if (!user_)
    {
        ...
        result_access_cache[0] = std::move(nothing_granted);
        return;
    }

    bool name_changed = false;
    if (user->getName() != user_->getName())
        name_changed = true;

    bool access_changed = false;
    if ((user->access != user_->access) || (user->access_with_grant_option != user_->access_with_grant_option))
        access_changed = true;
    ...
    if (access_changed)
        boost::range::fill(result_access_cache, nullptr);
}
...
bool AccessRightsContext::checkAccessImpl(Poco::Logger * log_, const AccessFlags & access, const Args &... args) const
{
    ...
    auto show_error = [&](const String & msg, [[maybe_unused]] int error_code)
    {
        if constexpr (mode == THROW_IF_ACCESS_DENIED)
            ...
            LOG_WARNING(log_, getUserName() + ": " + msg + formatSkippedMessage(args...));
    };

    if (!user)
    {
        show_error("User has been dropped", ErrorCodes::ACCESS_DENIED);
    }
