...
public:
    ...
    Node & operator =(const Node & src)
    {
        ...
        access = src.access;
        access_with_implicit = src.access_with_implicit;
    ...
    {
        getChild(name).grant(access_to_grant, subnames...);
        calculateAccess<false>();
    }
    ...
    void grant(const AccessFlags & access_to_grant, const std::vector<StringT> & names)
    {
        for (const auto & name : names)
            getChild(name).grant(access_to_grant);
        calculateAccess<false>();
    }
    ...
    AccessFlags getAccess(const std::string_view & name, const Args &... subnames) const
    {
        ...
        return child ? child->getAccess(subnames...) : access_with_implicit;
    }
    ...
    {
        mergeAccess(other);
        calculateGrantsAndPartialRevokes();
        calculateAccess();
    }
...
private:
    ...
    void calculateAccess()
    {
        ...
        if (children)
        {
            ...
            {
                /// Implicit grants.
                access_with_implicit |= AccessType::EXISTS;
                access_with_children |= AccessType::EXISTS;
            }
        }
    }

    void fullRevoke(const AccessFlags & access_to_revoke)
    {
        explicit_grants -= access_to_revoke;
        ...
        if (children)
        {
            for (auto & child : *children | boost::adaptors::map_values)
                child.fullRevoke(access_to_revoke);
        }
    }
    ...
    void mergeAccess(const Node & rhs)
    {
        ...
        if (children)
        {
            ...
            {
                lhs_child.inherited_access = access;
    ...
    void calculateGrantsAndPartialRevokes()
    {
        ...
        {
            for (auto & child : *children | boost::adaptors::map_values)
                child.calculateGrantsAndPartialRevokes();
        }
    }
};
