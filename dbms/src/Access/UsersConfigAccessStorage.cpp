    ...
    UserPtr parseUser(const Poco::Util::AbstractConfiguration & config, const String & user_name)
    {
        ...
        if (databases)
        {
            for (const String & database : *databases)
                user->access.grant(AccessType::ALL, database);
            user->access.grant(AccessType::ALL, "system"); /// Anyone has access to the "system" database.
        }
        else
        ...
        if (dictionaries)
        {
            user->access.fullRevoke(AccessType::dictGet, "");
            for (const String & dictionary : *dictionaries)
                user->access.grant(AccessType::dictGet, "", dictionary);
        }
        else if (!user->access.isGranted(AccessType::ALL))
            user->access.grant(AccessType::dictGet, "");
