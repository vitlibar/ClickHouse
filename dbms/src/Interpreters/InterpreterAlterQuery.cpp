    ...
    for (ASTAlterCommand * command_ast : alter.command_list->commands)
    {
        ...
        switch (command_ast->type)
        {
            ...
            case ASTAlterCommand::MOVE_PARTITION:
            {
                ...
                {
                    required_access.emplace_back(AccessType::COPY_PARTITION | AccessType::DROP_PARTITION, alter.database, alter.table);
                    required_access.emplace_back(AccessType::ATTACH_PARTITION, command_ast->to_database, command_ast->to_table);
                }
                break;
            }
