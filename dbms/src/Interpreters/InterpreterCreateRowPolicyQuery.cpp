...
BlockIO InterpreterCreateRowPolicyQuery::execute()
{
    ...
    std::optional<RoleList> roles_from_query;
    if (query.roles)
        roles_from_query = RoleList{*query.roles, access_control, context.getUserID()};
...
void InterpreterCreateRowPolicyQuery::updateRowPolicyFromQuery(RowPolicy & policy, const ASTCreateRowPolicyQuery & query, const std::optional<RoleList> & roles_from_query)
{
    if (query.alter)
    {
