    ...
    bool no_ddl_lock)
{
    ...
    if (database && table)
    {
        if (kind == ASTDropQuery::Kind::Detach)
        {
            ...
            database->detachTable(table->getTableName());
        }
        ...
        else if (kind == ASTDropQuery::Kind::Drop)
        {
            ...
            const std::string metadata_file_without_extension = database->getMetadataPath() + escapeForFileName(table->getTableName());
            ...
            database->removeTable(context, table->getTableName());
...
BlockIO InterpreterDropQuery::executeToTemporaryTable(const String & table_name, ASTDropQuery::Kind kind)
{
    ...
    else
    {
        ...
        if (table)
        {
            ...
            {
                context.checkAccess(table->isView() ? AccessType::TRUNCATE_VIEW : AccessType::TRUNCATE_TABLE, "", table_name);
            ...
            {
                context.checkAccess(table->isView() ? AccessType::DROP_VIEW : AccessType::DROP_TABLE, "", table_name);
...
AccessRightsElements InterpreterDropQuery::getRequiredAccessForDDLOnCluster() const
{
    ...
    {
        if (drop.kind == ASTDropQuery::Kind::Detach)
            required_access.emplace_back(AccessType::DETACH_DICTIONARY, drop.database);
        else if (drop.kind == ASTDropQuery::Kind::Drop)
            required_access.emplace_back(AccessType::DROP_DICTIONARY, drop.database);
    ...
    else
    {
        ...
        if (!drop.temporary && required_access.back().database.empty())
        {
            /// It can be temporary or not.
            required_access.push_back(required_access.back());
            required_access.back().setDatabase(AccessRightsElement::CurrentDatabaseTag{});
        }
    }
