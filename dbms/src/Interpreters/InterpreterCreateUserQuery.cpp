...
namespace
{
    void updateUserFromQueryImpl(User & user, const ASTCreateUserQuery & query, const std::optional<GeneralizedRoleSet> & default_roles_from_query = {}, const std::optional<UUID> & quota_from_query = {})
    {
        ...
        const GeneralizedRoleSet * default_roles = nullptr;
...
BlockIO InterpreterCreateUserQuery::execute()
{
    ...
    std::optional<GeneralizedRoleSet> default_roles_from_query;
    if (query.default_roles)
    {
        default_roles_from_query = GeneralizedRoleSet{*query.default_roles, access_control};
        if (!query.alter && !default_roles_from_query->all)
        {
            for (const UUID & role : default_roles_from_query->getMatchingIDs())
                context.getAccess()->checkAdminOption(role);
        }
    }
