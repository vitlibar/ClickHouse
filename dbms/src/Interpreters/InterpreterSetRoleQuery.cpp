...
void InterpreterSetRoleQuery::setRole(const ASTSetRoleQuery & query)
{
    ...
    {
        GeneralizedRoleSet roles_from_query{*query.roles, access_control};
...
void InterpreterSetRoleQuery::setDefaultRole(const ASTSetRoleQuery & query)
{
    ...
    auto & access_control = context.getAccessControlManager();
    std::vector<UUID> to_users = GeneralizedRoleSet{*query.to_users, access_control, context.getUserID()}.getMatchingUsers(access_control);
    GeneralizedRoleSet roles_from_query{*query.roles, access_control};
...
void InterpreterSetRoleQuery::updateUserSetDefaultRoles(User & user, const GeneralizedRoleSet & roles_from_query)
{
    if (!roles_from_query.all)
    {
        for (const auto & id : roles_from_query.getMatchingIDs())
