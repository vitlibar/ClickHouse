...
template class SettingsConstraints::ConstraintImpl<SettingUInt64>;
template class SettingsConstraints::ConstraintImpl<SettingInt64>;
template class SettingsConstraints::ConstraintImpl<SettingFloat>;
template class SettingsConstraints::ConstraintImpl<SettingMaxThreads>;
template class SettingsConstraints::ConstraintImpl<SettingSeconds>;
template class SettingsConstraints::ConstraintImpl<SettingMilliseconds>;
template class SettingsConstraints::ConstraintImpl<SettingLoadBalancing>;
template class SettingsConstraints::ConstraintImpl<SettingJoinStrictness>;
template class SettingsConstraints::ConstraintImpl<SettingTotalsMode>;
template class SettingsConstraints::ConstraintImpl<SettingOverflowMode<false>>;
template class SettingsConstraints::ConstraintImpl<SettingOverflowMode<true>>;
template class SettingsConstraints::ConstraintImpl<SettingDistributedProductMode>;
template class SettingsConstraints::ConstraintImpl<SettingString>;
template class SettingsConstraints::ConstraintImpl<SettingChar>;
template class SettingsConstraints::ConstraintImpl<SettingDateTimeInputFormat>;
template class SettingsConstraints::ConstraintImpl<SettingLogsLevel>;
...
SettingsConstraints::Constraint & SettingsConstraints::findOrCreateConstraint(const String & name)
{
    ...
    if (it == name_to_constraint_map.end())
    {
        ...
        {
            std::unordered_map<StringRef, std::function<std::unique_ptr<Constraint>()>> creator_map_;

#define ADD_ENTRY_TO_CREATOR_MAP(TYPE, NAME, DEFAULT, DESCRIPTION) \
            creator_map_.emplace( \
