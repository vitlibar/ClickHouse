...
public:
    ...
    void setMaxValue(const String & name, const Field & field);

    void check(const Settings & settings, const SettingChange & change) const;
    void check(const Settings & settings, const SettingsChanges & changes) const;
...
private:
    std::unordered_map<String, Field> max_values;
};
