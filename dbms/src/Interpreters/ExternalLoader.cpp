...
public:
    ...
    void addConfigRepository(Repository * repository)
    {
        std::lock_guard lock{mutex};
        repositories.emplace(repository, RepositoryInfo{repository->getName(), {}});
        need_collect_object_configs = true;
    }

    String removeConfigRepository(Repository * repository)
    {
        std::lock_guard lock{mutex};
        auto it = repositories.find(repository);
        if (it == repositories.end())
            return {};
        String repository_name = it->second.repository_name;
...
void ExternalLoader::addConfigRepository(IExternalLoaderConfigRepository * repository)
{
    ExternalLoader * ptr = nullptr;
    if (!repository->external_loader.compare_exchange_strong(ptr, this))
        return;
    config_files_reader->addConfigRepository(repository);
    reloadConfig(repository->getName());
}

void ExternalLoader::removeConfigRepository(IExternalLoaderConfigRepository * repository)
{
    ExternalLoader * ptr = this;
    if (!repository->external_loader.compare_exchange_strong(ptr, nullptr))
        return;
    const String repository_name = config_files_reader->removeConfigRepository(repository);
