...
public:
    ...
    void addDependency(const DatabaseAndTableName & from, const DatabaseAndTableName & where, AccessRightCheckingMode access_right_checking_mode = IGNORE_ACCESS_RIGHTS);
    void removeDependency(const DatabaseAndTableName & from, const DatabaseAndTableName & where, AccessRightCheckingMode access_right_checking_mode = IGNORE_ACCESS_RIGHTS);
    Dependencies getDependencies(const String & database_name, const String & table_name, AccessRightCheckingMode access_right_checking_mode = IGNORE_ACCESS_RIGHTS) const;

    /// Functions where we can lock the context manually
    void addDependencyUnsafe(const DatabaseAndTableName & from, const DatabaseAndTableName & where, AccessRightCheckingMode access_right_checking_mode = IGNORE_ACCESS_RIGHTS);
    void removeDependencyUnsafe(const DatabaseAndTableName & from, const DatabaseAndTableName & where, AccessRightCheckingMode access_right_checking_mode = IGNORE_ACCESS_RIGHTS);
