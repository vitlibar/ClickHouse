...
public:
    ...
    virtual void setQuota(const std::shared_ptr<QuotaUsageContext> & quota_) = 0;
...
public:
    ...
    void setQuota(const std::shared_ptr<QuotaUsageContext> & quota_) final { quota = quota_; }
...
private:
    LocalLimits limits;
    std::shared_ptr<QuotaUsageContext> quota;
