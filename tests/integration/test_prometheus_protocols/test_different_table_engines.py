...
# The columns of an external samples table (the layout of the samples table since version 2 of the TimeSeries table engine).
V2_SAMPLES_COLUMNS = (
    "(id UUID, "
...
def test_external_tables():
    node.query(
        "CREATE TABLE mysamples " + V2_SAMPLES_COLUMNS +
...
def test_data_keyword():
    ...
    node.query(
        "CREATE TABLE mydata " + V2_SAMPLES_COLUMNS +
        "ENGINE=AggregatingMergeTree ORDER BY (id, bucket)"
    )
    node.query(
        "CREATE TABLE mytags ("
