...
# `TAGS INNER COLUMNS` or `SAMPLES INNER COLUMNS` — the cross-source agreement check propagates the type
# to the other inner table.
def test_64bit_id():
...
def test_custom_id_algorithm():
    ...
    # Case 2: customize via the `id_generator` setting (id type is set by `TAGS INNER COLUMNS`).
    # Normalization should bake the setting's expression into the tags inner table's `id` DEFAULT.
    node.query(
        "CREATE TABLE prometheus ENGINE=TimeSeries "
