-- Tests the timeSeriesQuantileVaryingToGrid aggregate function: the per-grid-point 3rd argument, the value types it
-- accepts, and the requirement that it is the same array in every row and in every merged state.
...
SELECT timeSeriesQuantileVaryingToGrid(100, 120, 10, 30)(toDateTime(100), 10::Float64, [0., 0.5, 1.]); -- { serverError UNKNOWN_AGGREGATE_FUNCTION }
...
CREATE TABLE varying_input (grp UInt8, timestamp DateTime, value Float64, phis Array(Float64)) ENGINE = MergeTree ORDER BY timestamp;
INSERT INTO varying_input VALUES (0, 100, 10, [0, 0.5, 1]), (0, 110, 20, [0, 0.5, 1]), (1, 120, 30, [0, 0.5, 1]);

SELECT '-- timeSeriesQuantileVaryingToGrid: one quantile level per grid point';
...
SELECT '-- timeSeriesQuantileVaryingToGrid: the 3rd argument accepts Float32 as well as Float64';
SELECT timeSeriesQuantileVaryingToGrid(100, 120, 10, 30)(timestamp, value::Float32, phis::Array(Float32)) FROM varying_input;

SELECT '-- timeSeriesQuantileVaryingToGrid: partial states carrying the same arrays merge';
...
SELECT '-- timeSeriesQuantileVaryingToGrid: the 3rd argument must be the same in every row and in every merged state';
...
SELECT '-- timeSeriesQuantileVaryingToGrid: invalid arguments';
SELECT timeSeriesQuantileVaryingToGrid(100, 120, 10, 30)(timestamp, value, [0., 0.5]) FROM varying_input; -- { serverError BAD_ARGUMENTS }
SELECT timeSeriesQuantileVaryingToGrid(100, 120, 10, 30)(timestamp, value, [0, 1, 1]::Array(UInt64)) FROM varying_input; -- { serverError ILLEGAL_TYPE_OF_ARGUMENT }
SELECT timeSeriesQuantileVaryingToGrid(100, 120, 10, 30)(timestamp, value, phis::Array(Nullable(Float64))) FROM varying_input; -- { serverError ILLEGAL_TYPE_OF_ARGUMENT }
