...
FROM (SELECT arrayStringConcat(groupArray(explain), '\n') AS plan FROM (EXPLAIN actions = 1 SELECT sum(length(time_series)) FROM timeSeriesSelector(ts_lc, 'foo', 0, 1000)));
...
FROM (SELECT arrayStringConcat(groupArray(explain), '\n') AS plan FROM (EXPLAIN actions = 1 SELECT sum(length(time_series)) FROM timeSeriesSelector(ts_lc, 'foo{env="prod"}', 0, 1000)));
