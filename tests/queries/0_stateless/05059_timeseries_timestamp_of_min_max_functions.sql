...
-- IEEE-equal values (-0.0 and +0.0) are a tie resolved in favour of the latest sample: the value functions
-- return +0.0 (so 1/x is inf) and the timestamp functions return 110.
-- The first query uses the recompute path, the second one has a window of 64 buckets, which forces two-stacks.
SELECT 'ties';
WITH
    [100, 110]::Array(DateTime) AS timestamps,
    [-0., 0.]::Array(Float64) AS values
SELECT
    1 / (timeSeriesMinToGrid(110, 110, 0, 20)(timestamps, values))[1],
    1 / (timeSeriesMaxToGrid(110, 110, 0, 20)(timestamps, values))[1],
    timeSeriesTimestampOfMinToGrid(110, 110, 0, 20)(timestamps, values),
    timeSeriesTimestampOfMaxToGrid(110, 110, 0, 20)(timestamps, values);
WITH
    [100, 110]::Array(DateTime) AS timestamps,
    [-0., 0.]::Array(Float64) AS values
SELECT
    arrayDistinct(arrayMap(x -> 1 / x, timeSeriesMinToGrid(110, 120, 1, 64)(timestamps, values))),
    arrayDistinct(arrayMap(x -> 1 / x, timeSeriesMaxToGrid(110, 120, 1, 64)(timestamps, values))),
    arrayDistinct(timeSeriesTimestampOfMinToGrid(110, 120, 1, 64)(timestamps, values)),
    arrayDistinct(timeSeriesTimestampOfMaxToGrid(110, 120, 1, 64)(timestamps, values));
