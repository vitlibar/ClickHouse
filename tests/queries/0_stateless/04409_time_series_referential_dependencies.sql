...
DROP TABLE IF EXISTS recent_table;
DROP TABLE IF EXISTS recent_table_renamed;
...
DROP TABLE recent_table; -- { serverError HAVE_DEPENDENT_OBJECTS }
RENAME TABLE recent_table TO recent_table_renamed; -- { serverError HAVE_DEPENDENT_OBJECTS }

DROP TABLE ts_recent;
DROP TABLE recent_table;
...
DROP TABLE IF EXISTS ts_recent_inner;

CREATE TABLE ts_recent_inner ENGINE = TimeSeries SETTINGS recent_samples_ttl_seconds = 864000;

SELECT count() FROM system.tables WHERE database = currentDatabase()
    AND name LIKE '.inner\_id.recentsamples.%';

DROP TABLE ts_recent_inner;

SELECT count() FROM system.tables WHERE database = currentDatabase()
    AND (name = 'ts_recent_inner' OR name LIKE '.inner\_id.recentsamples.%');
