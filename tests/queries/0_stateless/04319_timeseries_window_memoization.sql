-- Tests the memoization in the recompute path (`fillGridResultsByRecompute`): the merged window aggregate is
-- reused across consecutive grid points that cover the same set of populated buckets. Here `buckets_per_window`
-- = 10 (<= 16, so the recompute path, not two-stacks, is used) and a window (1000) much wider than the data
-- extent (300..700) makes a run of grid points share the identical full bucket set (memo hits), while the
-- edges (buckets entering/leaving) force recomputes.
