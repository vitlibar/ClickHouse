...
$CLICKHOUSE_CLIENT -q "SELECT table, arraySort(dependencies_table), arraySort(referential_dependencies_table), arraySort(referential_dependents_table) FROM system.tables WHERE database=currentDatabase() ORDER BY table"
